...
const urls = [
    'https://docs.whatap.io/release-notes/service/service-1_104_x',
    'https://docs.whatap.io/release-notes/service/service-1_106_x',
    'https://docs.whatap.io/release-notes/service/service-1_108_x',
];
    ...
    for (const url of urls) {
        try {
            ...
            $('section.remark-sectionize-h2').each((index, element) => {
                ...
                let versionLink = '';
                if (versionMatch) {
                    let versionNumbers = versionMatch[0];
                    // 버전 문자열이 "1.108.00" 형태인 경우 "1.108"까지만 추출
                    const parts = versionNumbers.split('.');
                    if (parts.length > 2) {
                        versionNumbers = `${parts[0]}.${parts[1]}`;
                    }
                    const versionLinkPart = versionNumbers.replace(/\./g, '_');
                    versionLink = `https://docs.whatap.io/release-notes/service/service-${versionLinkPart}_x`;
                }
                ...
                $(element).find('h3').each((index, h3Element) => {
                    ...
                    const fileName = `./crw-data/crwld-service-test/_import-23q3-${lastUrl2}-${productName}.mdx`.trim();
                    ...
                    updateMDXContent(fileName, mdxContent, version, featuresByFeatureName, productName, lastUrl2, versionLink);
            ...
            Object.keys(productFiles).forEach(productName => {
                const filesList = productFiles[productName].map(file => {
                    ...
                    return `import ${productName}_23q3 from ".${cleanFileName}";\n\n<${productName}_23q3 />\n`;
                }).join('\n');
                const cleanProductName = productName.replace(/\u200B/g, '');
                productFilesContent += `## \`${cleanProductName}\`\n\n${filesList}\n`;
            });

            const newFilesMDX = `./crw-data/crwld-service-test/_import-new-files-${lastUrl2}.mdx`; // newFilesMDX 정의
...
function updateMDXContent(fileName, existingContent, version, newContent, productName, lastUrl2, versionLink) {
    ...
    Object.keys(newContent).forEach(featureName => {
        ...
        newContent[featureName].forEach(feature => {
            ...
            const newFeatureContent = `- ${feature} <code class='changelog-service'><a href="${versionLink}">${version}</a></code>`;
    ...
    const backupFileName = `./crw-data/backup/_import-backup-23q3-${lastUrl2}-${productName}.mdx`;
