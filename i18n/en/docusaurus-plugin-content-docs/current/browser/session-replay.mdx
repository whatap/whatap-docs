...
The default method to exclude collection of sensitive data is provided. All text data on the screen is masked and collected as follows:
...
Add `session-replay-ignore-class` to the class name of the html element for the area to be excluded from collection.
...
To apply the session replay feature, apply the browser agent option. You can set the `sessionReplaySampleRate` property in the `config` object from 0 to 100.
...
You can set the percentage of sessions to apply the session replay among the sessions to collect. For example, if `sampleRate` is **100** and `sessionReplaySampleRate` is **50**, only 50% of all sessions is recorded for the session replay.
...
<ImgLang img="browser-session-replay-user-session-flow.png" desc="User session flow" />
