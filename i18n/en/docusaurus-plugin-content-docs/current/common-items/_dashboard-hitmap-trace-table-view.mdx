...
- ![Column icon](/img/ico-column.svg) **_Select Column_**: You can add or hide the memory accumulation and CPU accumulation data as columns in the table list. The columns are as follows:

  - **_No_**: Step occurrence sequence

  - **_Time_**: Start time of each step

  - **_Gap_**: The waited time between the start time of the previous step and the current step may have a difference, while there may be a gap from the elapsed time if a delay happens due to external factors.
