...
1.  Check the class and method from the top items in order of share.

    -   Top stack with the **highest** 79.92% share<br/>
        `org.apache.logging.log4j.core.layout.TextEncoderHelper.copyDataToDestination()`
    -   Top stack with the **second highest** 8.01% share<br/>
        `sun.misc.Unsafe.park()` 

2.  You can check the trends for each time period with the histogram.
...
3.  Based on the confirmed market share, the expected effects of improvement are calculated.  
...
4.  Review the improvement plan based on the top stack information.
