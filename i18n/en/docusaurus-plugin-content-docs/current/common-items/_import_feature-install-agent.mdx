...
2. Create the **installation script**. 
   ...
   Execute the following command to automatically have the system detect the user environment and generate an installation script for the server with APACHE installed.

   ```bash
   curl http://repo.whatap.io/telegraf/feature/apache/install_apache_monitoring.sh -o install_apache_monitoring.sh
   ```

   Execute the following command to install the Apache agent.
