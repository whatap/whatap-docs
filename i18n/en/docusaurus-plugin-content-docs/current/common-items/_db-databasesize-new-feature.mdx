...
It displays **DATA SIZE** and **INDEX SIZE** for each table. PostgreSQL requires to set the following option in the DBX agent to collect table sizes.
...
You can download the searched data as a CSV format file. Select ![Download icon](/img/ic-csv-download-ms.svg) and then select **_Size details_** or **_Table size (TOP 50)_**.
