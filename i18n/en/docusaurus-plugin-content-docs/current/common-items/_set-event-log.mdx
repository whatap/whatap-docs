...
![Log Alert](/img/set-event-log-main.png)
...
:::note
To add or set an event, you must have the alert settings role. For more information about the role scheme, see [the following](../project/project-structure).
:::

## Common options for event addition

The following lists common options that can be set to add an event in **_Real-time Log Event_** or **_Composite Log Event_**.
...
-   **_Message_**: Enter an alert message to be displayed when an event occurs. You can apply variables to the message by entering `${tag or field key}`. The key to enter into the variable must be a value contained in the selected metric data, **_Category_**.<br/>
    If you enter `${logContent}` in a message, the bodies of the collected logs are output as the content of the warning alert.

    ![Log Event - Message](/img/set-event-log-message.png)
...
-   **_Event Receiving Tag_**: If you select an event receiving tag, alert notifications can be sent to project members and 3rd-party plug-ins with the tag. If the event receiving tag is not selected, alerts are sent to all project members.<br/>If the tag is not added, alerts are sent to all members. Click **_+ Add Tag_** and then select target alert receivers in **_Tag List_**. Select **_+ Create New Tag_** to add new tags.
...
![Real-time Log Event Alert Setting](/img/set-event-log.png)

-   **_Keyword search scope_**: The default value of this search key is **_All_**. **_All_** indicates the log's content.
...
-   **_Filtering event targets_**: Select values for **_Keyword search scope_**, **_Operator_**, and **_Keyword_** for filtering targets or select **_Direct Input_**. If there is no input, it determines whether or not to generate notifications for all data collected in real time.
    ...
    ![Filtering event targets](/img/set-event-log-filter.png)
...
![Composite Log Event Alert Setting](/img/set-event-complex-log.png)
...
> For example, among the logs in the `AppLog` category, the logs that meet `Enter conditions` are filtered. If `Exclude` is checked for condition, it indicates that the logs found under that condition are excluded. Thus, the logs whose `level` is `ERROR` are excluded. Alerts are sent when the event is less than `5` among the logs collected in the last 10 minutes.
...
-   If the **_An Event ID exists. Please retry after removing the ID first._** message appears, in the **_Import JSON_** window, delete `id` and then select **_+ Add to List_**.
