...
Log filters work only when the `log_filter_enabled` option is active. You can selectively collect logs via `log_fillter_allow_list` and `log_fillter_deny_list` when the value of the option is set to `true`. 

Log filters work on a blacklist basis. For example, if you enable `log_filter_enabled` and do not set any items to add to the `log_filter_allow_list` option, log collection for all containers is blocked by default. 
