You can search the logs containing keywords that have not been parsed, or for which no index has been created. In this case, all logs in the specified range are fully scanned. Therefore, the search speed may be slower compared to the indexed key. For standardized log data, it is recommended to search using the index key through <Cmdname sid="log_parser_setting" className="uitext" />.

<ImgLang img='log-filter-search.png' desc='Applying non-parsing keyword filters'/>

<InDoc pages="log-lt,aws-log-lt">

1. Select <Cmdname sid="category" className="uitext" />. <InDoc pages="log-search,aws-log-search">It is required to set the category.</InDoc>

2. In the <Cmdname sid="filter" className="uitext" /> field, enter a desired keyword for a search after leaving spaces on the `content` basis.
...
4. You can make additional inquiries by scrolling down and selecting <Cmdname sid="BTN07030" className="uitext" /> at the bottom. 
