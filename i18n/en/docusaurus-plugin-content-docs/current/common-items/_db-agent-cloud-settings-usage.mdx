...
DBXC provides stable execution, intuitive user experience, and extensive monitoring features.

| Feature | Description |
| --- | --- |
| **Stability** | - Prevents duplicate runs with PID files <br /> - Verifies successful start with health checks <br /> - 5-second startup timeout |
| **User Experience** | - Consistent color coding per project (agent) <br /> - Real-time log streaming <br /> - Intuitive status display |
| **Monitoring** | - Status API based on HTTP server (port: 53200) <br /> - Structured logging (Zap library) <br /> - Automatic log rotation |
...
- **start**: Start DBX agent  
- **stop**: Stop DBX agent  
- **restart**: Restart DBX agent  
- **logs**: View logs  
