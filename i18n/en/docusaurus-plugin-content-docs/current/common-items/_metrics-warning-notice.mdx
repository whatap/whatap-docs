...
- You can add metrics events from the <Cmdname sid="side_eventSetting" className="uitext" /> <code class="newfunc">New</code> menu, which has a new UI for improved usability. For more information, see [the following](metric-warning-notice-new).

- This feature is only available to the members with the <Cmdname sid="sms_alert_settings" className="b500" /> role. For more information about member roles, see [the following](../project/project-structure#project-auth).
...
For more information about the event templates for metrics, see [the following](#template).
...
- <Cmdname sid="level" className="uitext" />
  ...
  - It displays the alert level when an event occurs. The levels are <span class='vslow'>Critical</span>, <span class='slow'>Warning</span>, and <span style={{color:'#757575'}}>Info</span>. When setting the <span class='vslow'>Critical</span> and <span class='slow'>Warning</span> levels, the <Cmdname sid="CNT08320" className="uitext" /> selection option is enabled.

  - <Cmdname sid="CNT08320" className="uitext" /><ScrollToLink anchor='scrollTo-resolve-event' />: You can choose whether to send <Cmdname sid="CNT08320" className="uitext" /> that occurred among events. You can turn the feature on or off by selecting the toggle button.
...
- <Cmdname sid="alert_test" className="uitext" />
  ...
  - The <Cmdname sid="TTL07415" className="uitext" /> option sends test notifications only to users with the recipient tag set. If <Cmdname sid="ITM07419" className="uitext" /> is selected, the test notifications are sent to all users.
...
- <Cmdname sid="number_of_event" className="uitext" />: For the selected period, if the events set in <Cmdname sid="metrics_event_rules" className="uitext" /> occur as many as the input, an alert is sent.
  ...
  - If the option, <Cmdname sid="CNT08320" className="uitext" /> is activated, it is recommended to select <Cmdname sid="metrics_option_disabled" className="uitext" /> as the selected time.
...
You can check how many alerts have occurred by enabling the event conditions you set for the selected time period. If your select <Cmdname sid="run" className="uitext" />, the number of notifications occurred appears in the upper right corner. The selected field and thresholds are displayed on the chart in <Cmdname sid="metrics_event_rules" className="uitext" />.

:::note

- For more information about <Cmdname sid="metrics_event_rules" className="uitext" />, see [the following](#eventcondition).

- The <Cmdname sid="event_simulation_title" className="uitext" /> feature can test for the data for 24 hours.
