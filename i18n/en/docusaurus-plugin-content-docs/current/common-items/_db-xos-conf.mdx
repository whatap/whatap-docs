...
- **iplist_interval** <span class='type'>Second</span>
  ...
  If the `xos_dbip=true` option is set in the _whatap.conf_ file, it is sent to the `db_ip_list` category. You can check it after selecting the `db_ip_list` category from the <Cmdname sid="side_analysis" className="uitext" /> &gt; <Cmdname sid="side_metricsSearch" className="uitext" /> menu.
...
- **slow_query** <span class='type'>String</span>
  ...
  It collects slow queries from the database. If XOS is installed on the monitored DB but does not automatically recognize the file name, set `slow_query=` in the XOS _whatap.conf_ file.
