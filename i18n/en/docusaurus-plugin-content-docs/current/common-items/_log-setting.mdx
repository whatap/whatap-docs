...
Select a project in the initial screen of the WhaTap monitoring service, and then select **_Log_** > **_Setting_** under Project Menu. 
...
You can modify the log monitoring settings only when you have the **_edit permission_** on the project.

:::

![Log Setting](/img/log-setting.png) 

## ![](/img/number-01.png) Checking the guide and rate plan

In the ![number-s](/img/number-01.png) area, if you select **_Log monitoring guide_** and **_View plans_**, the related guide screen appears. 

## ![](/img/number-02.png) Checking the log monitoring

### Checking the agent settings

To start the log monitoring, check the agent version and settings. Click **_Check Agent Settings_** in the ![number-s](/img/number-02.png) area and follow the instructions.

-   **Application Monitoring**

       Under **_Apply_**, check the guides for each application. see the following guides: [Java](log-java), [PHP](log-php), and [Python](log-python).
...
In the ![number-s](/img/number-02.png) area, click **_Activate log monitoring_** to set whether or not to enable the log monitoring.

{@include: ../common-items/_log-java-start.mdx}

## ![](/img/number-03.png) Log Monitoring Data Setting

On the upper side of ![number-s](/img/number-03.png), you can check the **_log usage_**. You can also change the settings for **_Data Retention_** and **_Log lookup password_**.

### Data retention period

<!-- 수연님 편집 중 -->

This is the default data retention period to be applied commonly. If you do not set the data retention periods for each category, this data retention period is applied by default. If you set the data retention periods for each category and click **_Reset_**, the default data retention period is reset.

### Data retention periods for each category

<!-- 저장량 컬럼 삭제 -->

You can specify the log data retention periods for each category. **Log Count** means log lines stacked over the period. For example, **_Today Log Count_** is the number of log lines accumulated during the day, and **_Expected Log Count_** is the number of log lines of the today's log count multiplied by the days of data retention. 
...
For the **project manager**, the password must be changed to a new one in **_Setting_**.

:::

## ![](/img/number-04.png) Log Parser Setting

You can register a **parsing** **logic** to create a key for searching for the collected logs. If the parsing logic is not registered, searching is possible only with the following keys.
...
1.  On the lower part of the ![number-s](/img/number-04.png) area, click **_+ Add_**.

2.  In the **_parser selection field_** of ![number-s](/img/number-04.png), select a parser. For more information about the parser and pattern registration, see [the following](#log-pattern).

3.  In the **_category selection field_** of ![number-s](/img/ic-number-07.svg), select a category or directly input it. 

4.  You can add a filter by clicking ![Icon](/img/ico-filter.svg)**_filter_** of ![number-s](/img/ic-number-07.svg). If you enter a filter, the pattern is applied to the log data that meets the filtering condition.

5.  To check whether the pattern is normal, click **_Simulation_** of ![number-s](/img/ic-number-08.svg) and then measure the performance of simulation and pattern. For more information about simulation and performance measurement, see [the following](#parser-simulation).

6.  If the simulation is normal, in the ![number-s](/img/ic-number-08.svg) area, click **_Save_** to save it.

:::note

-   The addition of filters is optional.
-   If you click **_Reset_** in the ![number-s](/img/ic-number-08.svg) area, the written content is initialized.
-   By clicking ![number-s](/img/ic-number-09.svg) and then dragging the parser setting, you can change the sequences.
...
| Filtering | Enter values for `search key` and `search value` to apply as a filter. The pattern is applied only to the log data that meets the filter conditions.            | optional                     |
...
Four different types of parsers are provided as follows. You can check them in the parser selection field of ![number-s](/img/number-04.png).
...
![JSON Format Parser Pattern Registration](https://img.whatap.io/media/images/log_012_json_format_parser_1101.png)
...
-   **JSON custom pattern registration**
    ...
    ```
      ...
      To detect the JSON output part of a log, specify which part of a log to be recognized as JSON and parsed by combining the prefix and postfix options.
...
![4XX 5XX Status Code Parser](https://img.whatap.io/22/11/28/085706log_020_status_parser.png)

A parser that can be used if the status has already been parsed. Additional 4xx and 5xx status codes are parsed using the parsed status. You can create 4xx and 5xx counts with the parsed data. 
...
-   **status parser registration example**

      ![Grok parser - status parsing pattern registration](https://img.whatap.io/22/11/28/111724log_status_01_parser.png)

       If the incoming log is `{"msg":"message","status":404}` and the status is parsed by the Grok parser as in the example, parsing is performed like `status: 404`. If you confirm that the status has been normally parsed, register the 4XX, 5XX status code parser pattern.     
...
The status code success rate parser can be used when the status has already been parsed. For more information about the status parsing, see [the following](#4xx-5xx-status-parser). 
...
-   **Widget Templates**
      ...
      The data on the chart indicates the statistics during the query period. You can select the statistical method with the latest value, maximum value, or average value. The latest value is selected by default.
...
The parsed key can be checked in **_Live Tail_**, **_Search_**, and **_Explorer_**.
...
Log data is so large and difficult to retain for a long time. Using the **_Set log statistics data_** function, you can **save information on how many logs that meet specific conditions are collected every 5 minutes**. 
...
The setting is added as follows, statistical data is generated with a key value of TotalCount for the logs whose status is `200` or `300`. 

![](https://img.whatap.io/22/11/28/090932log_023_long_period_stat_config.png)

### Widget template

1.  In **_Widget Templates_** of **_Flex Board_**, search for **_Log long-term archive statistics_** to create a widget.

    ![Long-term Archive Statistics Widget Templates](https://img.whatap.io/22/11/28/090932log_024_long_period_widget_template.png)
2.  Select the **_category_** and **_key_** for data search and then click **_Apply_**.

    ![Long-term Archive Statistics Widget - selection of category and key](https://img.whatap.io/22/11/28/090932log_025_long_period_widget_config.png)
3.  With the added settings, you can check the long-term archive statistical data as follows:
