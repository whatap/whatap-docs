...
- **whatap.trace_user_enabled** <span class='type'>Boolean</span>
  ...
  Set whether or not to collect real-time users. If this value is activated, the users are traced based on the IP or PHP default cookie (PHPSESSID) and framework cookies (ci_session, Laravel_session). If your PHP compiled installation has a `whatap` entry in _php.ini_, add it to _php.ini_, not *whatap.ini*. 
