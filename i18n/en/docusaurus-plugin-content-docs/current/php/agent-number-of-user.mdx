...
-   **whatap.trace_user_using_ip**<span class='type'>Boolean</span><br/>
        Default `false`<br/>
        Set to classify real-time users by IP. The data can be checked in Real Time User. If the value is false, the users are classified based on the PHP default cookies and framework cookies (PHPSESSID, ci_session, Laravel_session). If your PHP compiled installation has a `whatap` entry in _php.ini_, add it to _php.ini_, not _whatap.ini_.
    {@include: ../common-items/_php-restart-apply.mdx}

-   **whatap.trace_user_cookie_keys**<span class='type'>String</span><br/>
        Default `PHPSESSID,ci_session, laravel_session`<br/>
...
-   **whatap.trace_user_header_ticket**<span class='type'>Boolean</span><br/>
        Default `false`<br/>
        Set the HTTP key to differentiate users by a specific value in the HTTP header. Note that the `trace_user_using_ip` setting cannot be applied simultaneously. If the HTTP key cannot be found, users can be identified based on cookies. If your PHP compiled installation has a `whatap` entry in _php.ini_, add it to _php.ini_, not _whatap.ini_.
    {@include: ../common-items/_php-restart-apply.mdx}
...
-   **whatap.trace_http_client_ip_header_key**<span class='type'>String</span><br/>
      Default `X-Forwarded-For`<br/>
      The function to set the client IP (remote IP) with a specific HTTP header value has been added. In the proxy environment, the X-Forwarded-For header value can be set to Client IP.
    {@include: ../common-items/_php-restart-apply.mdx}

-   **whatap.trace_http_client_ip_header_key_enabled**<span class='type'>Boolean</span><br/>
      Default `true`<br/>
      If the value is `false`, the HTTP Header is ignored and the client IP is set as Remote IP.
