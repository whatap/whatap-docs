...
- **use_env_first** <span class='type'>Boolean</span>
  ...
  The priority of agent configuration is that the *whatap.conf* file is applied first, followed by environment variables. To give priority to environment variables, set the option value to `true`. Alternatively, you can set the environment variable as `WHATAP_USE_ENV_FIRST=true`.
