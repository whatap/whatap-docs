...
-   **whatap.server.port** <span class='type'>tcp_port</span>
    ...
    Set the port of the collection server. Because only one port can be specified, the collection servers specified in whatap\_server\_host must use the same port.
