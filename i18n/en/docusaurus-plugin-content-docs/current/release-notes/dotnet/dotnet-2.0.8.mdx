...
- <Status>New</Status> Added the following agent setting options.
  ...
  - **profile_http_parameter_enabled** <span class='type'>Boolean</span>
    ...
    Set the security key to 6 digits in the WAS server's _`${WHATAP_AGENT_HOME}`/paramkey.txt_ file. If *paramkey.txt* does not exist, it is automatically generated as a random value.
