...
Method 1: Use the +load method (fastest)
...
```swift
let agent = WhatapAgentBuilder()
  .setProjectKey("<YOUR_PROJECT_ACCESS_KEY>")  // Required
  .setServerUrl("<YOUR_SERVER_URL>")   // Required
  .setPCode(<YOUR_PCODE>)             // Required 
...
```objectivec
WhatapAgentBuilder *builder = [[WhatapAgentBuilder alloc] init];
[builder setProjectKey:@"<YOUR_PROJECT_ACCESS_KEY>"];  // Required
[builder setServerUrl:@"<YOUR_SERVER_URL>"];  // Required
[builder setPCode:<YOUR_PCODE>];  // Required
