...
The number of active interfaces is displayed as **Connected** / **Total Interfaces**, allowing you to check how many interfaces are active. When you hover over a device, status information appears. When you click a device, the view opens **Interface status** if interface information is available; if not, it moves to <Cmdname sid="side_usageStatus" className="uitext" />.

* Example: `8 / 22` → 8 active out of 22 total

:::note

Interface information requires collecting SNMP ifTable (1.3.6.1.2.1.2.2) or ifXTable (1.3.6.1.2.1.31.1.1) data. Support may vary by device.

:::

### ICMP HealthCheck

The ICMP HealthCheck widget displays network jitter and maximum round trip time (Max RTT) in a graph. If the status is abnormal, the corresponding IP address is shown in <span class='vslow'>red</span>. Monitoring results are updated in 10 minute intervals.

1. Go to **Device inventory** and click the ![Edit icon](/img/ic-edit.svg) icon of the device you want to configure.  
1. In the device settings screen, configure it under the **Health Check** tab.

### Today’s Syslog

Syslog events generated today are displayed in a pie chart by severity level, categorized from Emergency to Debug. This lets you intuitively check log frequency and severity distribution.

### Top 10 devices by Trap events

Displays the devices with the most Trap messages in the last 24 hours, sorted in descending order by event frequency. This helps quickly identify devices with concentrated events.
...
Displays the interface with the maximum Packets Per Second (PPS) recorded in the last 24 hours.

* Example: `5.56K PPS (FastEthernet8, 2025-08-27 16:50:36)`

### Real time log events

Logs generated in the project are displayed in a real time table showing time, host, and log details. New logs appear instantly in live tail format. By analyzing logs in real time, you can quickly detect system failures and abnormal signs.