...
Data queried in MXQL uses different loading commands depending on the structure of the [metrics](../metrics/common-metrics-intro).

- **Tag-based Metrics**  
  Load data separated into tags and fields using the [`TAGLOAD`](mxql-loading#tagload) command.

- **Flat Structure Metrics**  
  Load metrics where all data is stored in fields using the [`FLEXLOAD`](mxql-loading#flexload) command.

- **Raw Log Data**  
  Use the [`LogSink`](mxql-loading#logsink) command to query raw log data.

- **Log Count Aggregation Data**  
  Use the [`LogSinkCount`](mxql-loading#logsinkcount) command to aggregate log counts by time unit.
