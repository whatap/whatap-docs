...
1.  Go to **_Permissions policies_** under the IAM console's **_Permissions_** tab. 

2.  Select **_forwarderpolicy_** in the policy list to go to the policy editing screen.

  ![IAM Console](/img/whatap-forwarder-vpc-aws-iam.png)

1.  Select **_JSON_** tab and then copy and paste the following policy. The policy creates, defines, and deletes _ENI_.
...
1.  Seeing the following, set **_VPC_**, **_Subnets_**, and **_Security groups_**, and then select **_Save_** to save the results.
...
```
* Connect the Internet via **IGW** using the **Public IP** of **NAT Gateway**.

* To connect the Internet via **IGW**, **Public IP** is required. Lambda's *ENI* can allocate the **Public IP**.
