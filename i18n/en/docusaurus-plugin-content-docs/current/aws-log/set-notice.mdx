...
You can set alerts by filtering the collected log data according to the conditions. Select **_+ Add alert policy_** to set the log event alert. After all settings are finished, select **_Save_**. The **_Real-time log event_** can be set. Alerts are sent when the keyword is found in the logs collected in real time.
