---
id: agent-log
title: Agent log configuration
description: The options of the agent log can be set.
tags:
  - Node.js
  - Log
---

When the application server runs, it starts collecting the monitoring data. The collected data is stored in the log file.

-   **log_rotation_enabled** <span class='type'>Boolean</span><br/>
     Default `true`<br/>The function to store agent log files by date is activated. The log file name is stored in the format of _whatap-'yyyymmdd'.log_. If the value is `false`, the log file name is stored in the format of _whatap.log_.

-   **log_keep_days** <span class='type'>Int</span><br/>
     Default `7`<br/>
     It sets the log file retention period. It works only when `log_rotation_enabled` is `true`.

-   **log_datasource_lookup_enabled** <span class='type'>Boolean</span><br/>
    Default `true`<br/>
    The function to record the log is activated in case of DataSource upon InitialContext Lookup.
