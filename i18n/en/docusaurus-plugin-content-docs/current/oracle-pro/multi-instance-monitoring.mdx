...
If you are using Oracle Real Application Cluster (RAC), click the service name in <Cmdname sid="ITM07760" className="uitext" /> and select the ***Default(RAC)*** preset from the presets to monitor. You can check the metrics based on the instances running on the RAC database.
...
For more information about the <Cmdname sid="session_detail" className="uitext" /> window, see [the following](usage-ui#checksessioninfo).
...
- For more information about the <Cmdname sid="TTL06224" className="uitext" /> window, see [the following](usage-ui#query-details).
