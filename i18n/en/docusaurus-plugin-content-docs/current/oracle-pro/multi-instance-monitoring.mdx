---
id: multi-instance-monitoring
title: Monitoring Multiple Instances
description: It provides the function to display the integrated data for key metrics of multiple database instances.
keywords:
  - Oracle Pro
  - Instance
  - Dashboard
  - DATABASE
  - Monitoring Multiple Instances
---

import Dbv2mulitinstancemonitoringdesc from "../common-items/_db-v2-mulit-instance-monitoring-desc.mdx";

<Dbv2mulitinstancemonitoringdesc />

<ProdImg img='db-multi-instance.png' desc='Multi-instance Monitoring' />

import Dbmultiinstancemonitoringdescnotes from "../common-items/_db-multi-instance-monitoring-desc-notes.mdx";

<Dbmultiinstancemonitoringdescnotes />

import Apmdashboardsetpasttime from "../common-items/_apm-dashboard-set-past-time.mdx";

<Apmdashboardsetpasttime />

import Apmdashboardcheckagent from "../common-items/_apm-dashboard-check-agent.mdx";

<Apmdashboardcheckagent />

import Apmdashboradeditwidget from "../common-items/_apm-dashborad-edit-widget.mdx";

<Apmdashboradeditwidget />

import CustomWidget from "../common-items/_db-mim-custom-widget.mdx";

<CustomWidget />

import Apmdashboardwidgetoptions from "../common-items/_apm-dashboard-widget-options.mdx";

<Apmdashboardwidgetoptions />

import Apmdashboardpreset from "../common-items/_apm-dashboard-preset.mdx";

<Apmdashboardpreset />

import WidgetDetails from "../common-items/_db-v2-mim-widgets.mdx";

<WidgetDetails />

import Dbv2mimactivesessiondesc from "../common-items/_db-v2-mim-activesession-desc.mdx";

<Dbv2mimactivesessiondesc />

import Dbdashboardactivesessiondesc from "../common-items/_db-dashboard-activesession-desc.mdx";

<Dbdashboardactivesessiondesc />

import Dbdashboardlocktreedesc from "../common-items/_db-dashboard-locktree-desc.mdx";

<Dbdashboardlocktreedesc />

import Dbv2mimactivesessionnote from "../common-items/_db-v2-mim-activesession-note.mdx";

<Dbv2mimactivesessionnote />

### Checking the session information details {#session-details}

If you select a session in the <Cmdname sid="active_sessions" className="uitext" /> tab, the <Cmdname sid="session_detail" className="uitext" /> window appears. You can see the information details about the selected session.

<ImgLang img='db-session-details-info-oracle-pro.png' desc='Session Details' />

:::note

For more information about the <Cmdname sid="session_detail" className="uitext" /> window.

:::

### See query details {#query-details}

When you select the query column item in the result list, the **SQL Details** window appears.  
Click the <Cmdname sid="BTN06225" className="uitext" /> **→** button to move to the <Cmdname sid="statistics_sql" className="uitext" /> menu, where you can check statistical information related to the SQL query.

<ImgLang img="db-sql-details-oracle-pro.png" desc="SQL Details" />

The tabs in the **SQL Details** window are as follows:

* **Trend**: Provides a comprehensive analysis of the execution time, I/O, and wait events of a specific SQL, allowing quick identification of performance bottlenecks.
* **Runtime Plan**: Displays the execution plan and runtime information of the selected SQL query, including execution count, average execution time, and average physical reads.
* **Explain Plan**: Shows the execution plan predicted by the optimizer, including cost, operations, object names, and cardinality.
* **Plan History**: Displays the history of execution plans for SQL queries executed in the database.
* **Bind Capture**: Shows the values of bind variables used in SQL queries executed in the database, helping you understand the actual execution content of the query.

:::note

You can see the tuning guide after selecting the ![Tuning icon](/img/ic-chat-red.svg) button in the Plan tree. The tuning guide can be viewed when the nodes in the Plan Tree are `TABLE ACCESS (FULL)` and `INDEX (SKIP SCAN)` and the nodes contain the keyword, `CARTESIAN`.

:::

import Dbtabledatafiltering from "../common-items/_db-table-data-filtering.mdx";

<Dbtabledatafiltering level={3} />

import Dbv2dataeditcolumn from "../common-items/_db-v2-data-edit-column.mdx";

<Dbv2dataeditcolumn level={3} />

{/* 컬럼 정보 안내 */}
import Oraclecolumn from "../common-items/_db_oracle-column.mdx";

<Oraclecolumn level={3} />

import Dbdashboardsessiontabledownload from "../common-items/_db-dashboard-session-table-download.mdx";

<Dbdashboardsessiontabledownload />

import Dbdashboardsessiontablenew from "../common-items/_db-dashboard-session-table-new.mdx";

<Dbdashboardsessiontablenew />
