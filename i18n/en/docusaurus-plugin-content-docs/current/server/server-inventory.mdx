...
Click the filter input field, select the desired **filter key** and **condition**, enter values, and then click **Apply**. In the server inventory list area, you can view the filtered list.

<ServerInventoryFilter />

### Align

Sorting options allow up to 3 levels of multi-sorting. You can sort by any desired criteria including custom columns. The default settings for sorting are <Cmdname sid="TTL08087" className="uitext" /> &gt; <Cmdname sid="TTL08099" className="uitext" /> &gt; <Cmdname sid="TTL08100" className="uitext" />. 
...
You can track server changes by checking past data in the server inventory, viewing change history, and comparing by date.

#### Past Data Lookup

You can load snapshot data saved at midnight each day to check past data or the change history of specific fields.

1. Click the YYYY/MM/DD ![Calendar icon](/img/ic-start-date.svg) button.

1. Select the past date you want to query, then click the **OK** button to view the data.

:::caution

When viewing past data, the **Edit**, **CSV Upload**, and **Column Settings** functions are not available.  
The Connection field in the table is automatically hidden.

:::

#### Change History Lookup

You can check how the value of a specific field in the selected server changed by date during the query period, as well as view creation and deletion history of that field.

1. In <Cmdname sid="side_serverInventory" className="uitext" />, click the ![Time icon](/img/ico-timing.svg) **Change History Lookup** button.

1. In the **Inventory Change History Search** window, select the date, server, and column name to query.

1. Review the change history by date according to the selected conditions.

#### Date Comparison

You can compare two dates of a selected server to check data changes and whether fields were created or deleted.

1. Click the **History** ![Time icon](/img/ico-timing.svg) icon of the server to compare.

1. In **History Lookup**, select the **Base Date** and **Comparison Date** to compare the data.

### Column Settings

Click ![Column Icon](/img/ic-column.svg) <Cmdname sid="column_settings" className="uitext" />. Select columns and add custom columns as shown below, and then select <Cmdname sid="BTN06053" className="uitext" /> at the bottom to save the column settings. 
...
By selecting the ![Download icon](/img/ico-download.svg) <Cmdname sid="download" className="uitext" /> button, you can download the collected server inventory in *CSV* file format.
...
1. Click the ![Upload icon](/img/ico-upload.svg) **CSV** <Cmdname sid="upload" className="uitext" /> button, then select the *CSV* file to upload.
...
Select the ![Edit icon](/img/ic-edit.svg) <Cmdname sid="edit" className="uitext" /> button to edit the server inventory list.
