...
Only supported on Linux. Requires agent version 2.9.2 or higher.
...
### ![Filter Search Icon](/img/ico-filter.svg) Keyword Search Filter

By using the filter search function, you can quickly check only the GPU instances that match your desired conditions.  
Enter the filter in the ![Filter Search Icon](/img/ico-filter.svg) search box and press Enter to search.
