...
Run the *start.sh* shell script file or the *start.bat* batch file from the installation path of the database agent. When you run the agent, it starts collecting monitoring data from the database. Check the agent log to see whether the agent has started.
...
To check that the log has been normally uploaded from the server and the console has been registered normally, go to the initial screen of [Whatap Monitoring Service](https://service.whatap.io). Select the project to check and then select ***Management*** > ***Agent List*** on the left. From ***Agent List***, you can check the name of the database server. To manage the database server names separately, see [the following](agent-manage).
