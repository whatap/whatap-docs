...
It can happen when using MySQL 8.0, and its default authentication plug-in is `caching_sha2_password`. To use the `caching_sha2_password`, use the SSL-secured connection or a non-encrypted connection with the RSA security applied. The easiest way to solve this problem is to use the `mysql_native_password` when generating passwords as follows.
