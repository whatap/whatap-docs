...
| ^            | `grant select on performance_schema.* to whatap;`                                               | Active session collection / <Cmdname sid="side_dbLockTree" className="uitext" /> / <Cmdname sid="TTL06865" className="uitext" />                      | `performance_schema.data_locks`<br/>`performance_schema.threads`<br/>`performance_schema.events_statements_summary_by_digest`, etc.                         |
...
If you use the JDBC driver version 8.x or later, you may need to set `serverTimezone` for the connection URL. Set the following options in the _whatap.conf_ file:
