...
Log in [Whatap Monitoring Service](https://service.whatap.io/) and then select a project. Select **_Application_** > **_Instance Performance Analysis_**, and then select the **_Runtime Boot Environment_** tab. Check the **_whatap.name_** and **_whatap.type_** entries. The last element of the **_whatap.name_** entry must be the container name. The application server's type must be specified in the **_whatap.type_** field.
...
| -------- | :-----------------------------: | :-----------------------------: | :-----------------------------: |
| JEUS 7   | <span class='emoji-ok'>✅</span> | <span class='emoji-ok'>✅</span> | <span class='emoji-no'>❌</span> |
