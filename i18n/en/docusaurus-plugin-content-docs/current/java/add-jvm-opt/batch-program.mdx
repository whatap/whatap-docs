...
⎿ *whatap.conf*: Batch host configuration file (If there is no file, create a new one to add the items for `license` and `whatap.server.host` in **_Setting_** > **_Agent installation_**.)
