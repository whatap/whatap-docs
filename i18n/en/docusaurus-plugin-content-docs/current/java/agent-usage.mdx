...
- **oshi_enabled** <span class='type'>Boolean</span> <code class="my">Java Agent v2.1.0 or later</code>
  ...
  It is recommended to use it in the environment with Java 8 or later.
...
- **sigar_enabled** <span class='type'>Boolean</span>
  ...
  - In Java 8 or earlier or Java 1.6 or earlier, you can enable the sigar library and collect performance metrics by adding the `sigar_enabled=true` option.

  - Depending on the application environment, you may see CPU metrics displayed as **-1**. You can check the recommendations provided by WhaTap and then resolve the issues. See [the following](agent-troubleshooting#cpumetrics).
