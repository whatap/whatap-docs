...
1. **Install the WhaTap Operator**
    ...
    * **Example**
    ...
    If you use a custom configuration instead of the default *values.yaml*, install the operator with the following commands.
...
When installing the WhaTap agent with the operator, refer to [WhaTap CR configuration options](whatap-cr-configuration-options) for details on CR settings (resource constraints, tolerations, imagePullSecret).
