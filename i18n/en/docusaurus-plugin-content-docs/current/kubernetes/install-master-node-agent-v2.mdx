...
In the WhaTap Kubernetes agent installation _YAML_ file, add `customResource` to use in the `resources` under `rules` of the `ClusterRole` resource. 
...
When installing the WhTtap Kubernetes agent by using the Helm chart, add the custom resource names to be monitored in the default _values.yaml_ file. Add the custom resource name in `resources` as follows: 
