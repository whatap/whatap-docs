...
- The volume type provided by WhaTap Kubernetes agent is as follows: 

  > `HostPath`, `ConfigMap`, `Projected`, `EmptyDir`, `PersistentVolumeClaim` (PVC), `Secret`

- You can see the types of volumes supported by Kubernetes in the [Official Kubernetes documentation](https://kubernetes.io/ko/docs/concepts/storage/volumes/).
...
The summary analysis of the current node is available only in Korean. 
...
- Service networking info: You can see the connected service and Ingress information. 
