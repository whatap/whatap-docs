...
- **collect_kube_node_process_metric_target_list** <span class='type'>list</span> <ScrollToLink anchor='collect_kube_node_process_metric_target_list' />
  ...
  **List of default monitoring target processes by WhaTab**
  ...
  - **aws-k8s-agent** 
    ...
    {/* 이 프로세스는 포드 간의 네트워크 트래픽을 처리하기 위해 Elastic Network Interface(ENI)를 동적으로 할당 및 해제하는 작업을 수행합니다. It plays an important role in debugging the issues related to network connection failures. */}
...
{/* 
  ...
  **List of processes that can be monitored**
...
- **containerd-shim** 
  ...
  The process plays as an intermediate interface between `containerd` and container runtime. This process manages the lifecycle of each container and is responsible for handling standard input/output and collecting logs for the container.
...
- **dockerd** 
  ...
  The Docker daemon is a core process that oversees building, deploying, and running container images. It provides various features including network management between containers and data volume management.
...
- **aws-k8s-agent** 
  ...
  This agent manages Kubernetes network interfaces (VPC CNI) in the AWS environment. This process dynamically allocates and de-allocates elastic network interfaces (ENIs) to handle network traffic between Pods. It plays an important role in debugging the issues related to network connection failures.

- **kube-proxy** 

  This process routes network traffic between Pods in the cluster and relays communication between services and Pods. By using the Linux `iptables` or IPVS, it delivers network requests in the cluster to the correct Pods, and supports service discovery and load balancing.

  :::

- **kubelet:**\
  It communicates with the Kubernetes master, manages the state of containers on nodes, and helps make sure they are deployed and run correctly.

- **containerd-shim:**\
  This interface between the containerd and container runtime handles the lifecycle and logs of the container.

- **containerd:**\
  It is a container runtime that handles container execution, image management, networking, etc.

- **dockerd:**\
  It is a core process for Docker container management, and performs container image building, execution, deployment, networking, data management, etc.

- **crio:**\
  This container runtime manages containers in the Kubernetes environment.

- **coredns:**\
  It provides DNS services in the Kubernetes cluster, supporting the name resolution, service discovery, and load balancing.

- **aws-k8s-agent:**\
  It manages the Kubernetes VPC CNI in the AWS environment, and allocates and releases AWS network resources for inter-Pod communication.

- **kube-proxy:**\
  It routes network traffic in the cluster, supporting service discovery and load balancing via iptables or IPVS. 
...
| Option name                                | Default          | Type   | Description                                                                                                | Remarks                                                                                                                                                                                          |
| ------------------------------------------ | ---------------- | ------ | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `collect_volume_detail_enabled`            | `true`           | bool   | It enables querying the container volume using the `exec df` command for all containers by the node agent. | It enables the feature to collect the storage usage and volume for all containers running on the node.                                                                                           |
| `inject_container_id_to_apm_agent_enabled` | `true`           | bool   | It enables container ID injection using `exec` in the WhaTap APM container                                 | The ID of a container identified by WhaTap APM can be automatically extracted and injected into the APM tool, which can be used to associate the performance monitoring data with the container. |
| `debug`                                    | `false`          | bool   | Whether the debug mode is active or not                                                                    | Set the log level to `DEBUG` when the debugging mode is enabled, otherwise to `INFO`.                                                                                                            |
| `kube_config_path`                         | `~/.kube/config` | string | Kubernetes configuration file path                                                                         | If not set, use _~/.kube/config_.                                                                                                                                                                |
| `log_sys_out`                              | `true`           | bool   | Setting the log system output                                                                              | When it is set to `true`, logging is left simultaneously in the standard output (stdOut) and a file. Any changes are immediately reflected in the logging system.                                |

whatap-node-helper 

| Option name                          | Default                                             | Type   | Description                                                                               | Remarks                                                                                                                                                                                                            |
| ------------------------------------ | --------------------------------------------------- | ------ | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `shutdown`                           | `false`                                             | bool   | Forcibly disconnect the collection server (communication stop).                           | -                                                                                                                                                                                                                  |
| `license`                            | -                                                   | string | Agents are identified using the license issued to the project.                            | -                                                                                                                                                                                                                  |
| `whatap.server.host`                 | `13.124.11.223` or `13.209.172.35`                  | string | IP of the collection server.                                                              | `delimiter` `/:`                                                                                                                                                                                                   |
| `whatap.server.port`                 | `6600`                                              | number | Data receiving port of the collection server.                                             | -                                                                                                                                                                                                                  |
| `ONODE_NAME`                         | `NODE_NAME` or `NODE_IP` environment variable value | string | Name to identify the agent node.                                                          | The environment variable (`NODE_NAME` or `NODE_IP`) is used.                                                                                                                                                       |
| `tcp_connection_timeout`             | `3000`                                              | number | Timeout time when connecting whatap-master-agent to the collection server socket.         | -                                                                                                                                                                                                                  |
| `tcp_so_timeout`                     | `60000`                                             | number | Data sending/receiving timeout time after socket connection.                              | -                                                                                                                                                                                                                  |
| `debug`                              | `false`                                             | bool   | Whether the debug mode is active or not                                                   | -                                                                                                                                                                                                                  |
| `gpuEnabled`                         | `false`                                             | bool   | Whether GPU monitor is active or not.                                                     | -                                                                                                                                                                                                                  |
| `count_interval`                     | `5000`                                              | number | Cycle to transmit metrics (`kube pod`, `container`) to the collection server.             | This option may affect the on-screen data refresh cycle. For more information, see [Screen components affected by each metric](#k8s-affected-comp-by-metrics). (unit: ms)                                          |
| `collect_interval`                   | `5000`                                              | number | Cycle for the agent to request apiserver to collect metrics (`kube_pod`, `container`).    | This option may affect the on-screen data refresh cycle. For more information, see [Screen components affected by each metric](#k8s-affected-comp-by-metrics). (unit: ms)                                          |
| `collect_apiserver_metric_by_leader` | `false`                                             | bool   | If the option is enabled, the node agent delegates metrics collection to the master agent | It requires to enable the `collect_apiserver_metric_by_leader` option of whatap-master-agent. Through metric collection delegation, the apiserver load generated by the existing whatap-node-agent can be reduced. |
