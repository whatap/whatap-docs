...
- **profile_sql_param_enabled** <span class='type'>Boolean</span><ScrollToLink anchor='profile-sql-param' />
  ...
  Specify the security key to a 6-letter value in the WAS server's _`%PROGRAMFILES%`/WhaTap .NET\paramkey.txt_ file. If *paramkey.txt* does not exist, it is automatically created as a random value.
