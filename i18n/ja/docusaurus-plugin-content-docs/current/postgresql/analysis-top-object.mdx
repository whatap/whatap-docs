...
| `vacuum_count`        | 手動でバキューム(vacuum)した回数(VACUUM FULLは除外)            |
| `n_mod_since_analyze` | 最後の分析(analyze)以後変更された行数(推定値)                    |

## Age

PostgreSQLからはXID(transaction id)を循環的に使用して、ある時点でXIDがラップアラウンド(Wraparound)されることがあります。 これを防止するために`xid_age`(Current XID - 作成時点のXID)が増え続けないように管理されなければなりません。 `autovacuum_freeze_max_age`を超過するとAnti-Wraparound Vacuumを自動的に行うようになり`autovacuum_freeze_max_age`以下で`xid_age`を管理します。
