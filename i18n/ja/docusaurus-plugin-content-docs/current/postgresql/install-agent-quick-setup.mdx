...
- **Connecting DBMS Server with SSL**：SSLプロトコルを利用してデータベースサーバーに接続する場合、`true`を入力してください。 既定値は`false`です。

## エージェントを実行する

_setup.sh_ファイルを利用してエージェントのインストールプロセスを完了したら、データベース情報と*whatap.conf*ファイルに設定した内容が出力されます。
