...
WCFおよびWeb Serviceの追跡用エージェント設定ファイル(_whatap.conf_)についての例を参照してください。
...
これは、エージェント設定ファイル(_whatap.conf_)で指定した`webservice_method_prefix`の値(`Whatap.DotNet.Examples.Service1`)から始まるため、トランザクションとして扱います。
