...
-   エージェント名は、**Whatap .NET**サービスが再起動するまで有効になりません。 
...
-   **app_process_name** <span class='type'>String</span>
    ...
    アプリケーションを識別するためのエージェント名(ONAME)の構成要素です。アプリケーションのプロセス名でアプリケーションサーバーのCPU、Heap Memoryなどを収集する対象プロセスを設定します。`object_name`オプション`{process}`に対応します。
