...
セッションリプレイ機能を適用するには、ブラウザエージェントオプションを適用する必要があります。 次の例題を参照してください。
...
- **sessionReplaySampleRate** <span class='type'>Int</span>
  ...
  全収集セッションのうち、セッションリプレイ収集比率として0から100まで設定できます。 例えば、`sampleRate`が**100**であり、`sessionReplaySampleRate`が**50**であると、全体セッションの中で50%のみセッションリプレイとして記録されます。
...
- **sessionReplayMaskAllTexts** <span class='type'>Boolean</span>
  ...
  値を`false`に設定すると、マスキング処理なしですべてのテキスト データを収集します。
...
機微情報に対する収集の除外方法を基本的に提供します。 画面上のすべてのテキストは、次のようにマスキングされ、収集されます。 ただし、エージェントオプションで`sessionReplayMaskAllTexts`の値が`true`に設定されている必要があります。
...
|                      特定領域に対するマスキング処理                     |                        マスキングを適用していない場合                       |
| :------------------------------------------------------: | :----------------------------------------------------------: |
| ![マスキング処理](/img/browser-session-replay-area-masking.png) | ![マスキング未処理](/img/browser-session-replay-no-area-masking.png) |

収集対象から除外する領域についてのhtml要素(element)のクラス(`class`)名に`session-replay-block-class`を追加します。 次の例題を参照してください。
...
特定のテキスト領域を収集対象から除外するには、該当html要素(element)のクラス(`class`)名に`session-replay-mask-text-class`を追加します。 次の例題を参照してください。
...
記録されたユーザー行動イベントを再生して、実際にユーザーが画面をどのように操作したかを確認できます。
...
<Cmdname sid="TTL08103" className="uitext" />画面の右側で実際のユーザーのセッション情報を確認できます。
