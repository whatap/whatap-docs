...
WhaTapブラウザーエージェントは、ユーザーのブラウザーから発生したイベントを収集し、収集したデータを再加工してモニタリングに活用します。 すべての収集データは、ユーザーセッションに従属しています。
...
| データ名          |                タイプ               | 説明                                                                                                  | 例                                                                                                                                 |
| ------------- | :------------------------------: | --------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| sessionID     |  <span class='type'>Long</span>  | ウェブアプリケーションユーザーがウェブページの探索を開始する際に発行する固有IDです。 セッションの保持期間は最大4時間で、15分間活動がなければIDは満了します。 ブラウザークッキーに保存します。 | x7dbvq64ga80sd                                                                                                                    |
| userID        |  <span class='type'>Long</span>  | ウェブアプリケーションユーザーが初めて接続するときに発行する固有IDで、有効期限がありません。 ブラウザークッキーに保存します。                                    | z7eqnnitlc19j9                                                                                                                    |
...
ユーザーがウェブアプリケーションにアクセスし、ページのコンテンツを完全に読み込んで発生するイベントです。 基本的な収集データは次のとおりです。

<div class='br-data'>

| データ名                               |                      タイプ                     | 説明                                                                   | 例                           |
| ---------------------------------- | :------------------------------------------: | -------------------------------------------------------------------- | --------------------------- |
| navigationTiming.startTimeStamp    |      <span class='type'>TimeStamp</span>     | `navigationStart`のタイムスタンプです。 ページを読み込み始める時間のタイムスタンプ値です。               | 1658057382311               |
...
| navigationTiming.domInteractive    | <span class='type'>HighResolutionTime</span> | `navigationStart`からブラウザーがdocumentの構文解析を完了した時刻です。 DOMが用意されているステータスです。 | 441                         |
...
ユーザーがSingle Page Application(SPA)形態のウェブアプリケーションでページ移動時に発生するイベントです。 基本的な収集データは次のとおりです。
...
**コアウェブバイタル**(Core Web Vitals)は、ウェブコンテンツ利用者の使用経験に影響を及ぼす多様な測定値の中でグーグルが重要だと強調する3つのメトリクスです。 基本的な収集データは次のとおりです。
