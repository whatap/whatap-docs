...
1.  Pythonアプリケーション起動時、WhaTapエージェントがInjectionできるように、アプリケーションの起動スクリプトを変更します。 
    ...
    ```python
    export WHATAP_HOME={ログと構成ファイルのパス}
    ```

2.  `WHATAP_HOME`で設定したパスに_whatap.conf_ファイルを作成および設定するように次のコマンドを実行してください。

    ```bash
    whatap-setting-config \
    --host {収集サーバーIP} \
    --license {アクセスキー} \
    --app_name {ユーザー指定アプリケーション名} \
...
サービス実行ファイル(_/etc/supervisor/conf.d/supervisor.conf_)を修正し、エージェント開始コマンドでアプリケーションを開始してください。ユーザー環境によって、サービス実行ファイルのパスが異なる場合があります。
