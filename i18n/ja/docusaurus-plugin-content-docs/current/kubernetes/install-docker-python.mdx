...
アプリケーションを実行する前に、Pythonエージェントの作業ディレクトリを設定し、デフォルトの設定ファイルを作成します。設定ファイル(_entrypoint.sh_)を通じてエージェントに認証情報やログの追跡有無などを設定します。
...
# 以下は、エージェント構成に必須な設定値です。アプリケーション配布のyamlファイルを通じて設定します。
...
2. 「WHATAP_HOME」に設定したパスに_whatap.conf_ファイルを作成するため、次のコマンドを実行してください。「$」で表記された変数は、エージェント構成に必要な設定値です。アプリケーション配布の_yaml_ファイルを通じて設定します。
...
- _yaml_の作成例は、次の[コンテナ環境変数の設定](#k8s-env)をご参照ください。
...
次のDockerfile定義の例をご参照ください。_entrypoint.sh_スクリプトを実行する例です。
...
次の設定は、オプションとして必要な場合のみ使用してください。設定ファイル(_entrypoint.sh_)に次の追加設定の他にも、ログおよびトランザクション関連の設定が可能です。その他の追加設定に関しては、[次の文書](https://docs.whatap.io/python/set-agent)をご参照ください。 
...
- `app_name`：アプリケーションを識別するためのエージェント名の構成要素です。エージェント識別の詳細については、[次の文書]](https://docs.whatap.io/python/agent-name)を参考にしてください。

- `app_process_name`：CPU、Heap Memoryなど収集する対象プロセスを設定します。例としてuwsgi、gunicornなどがあります。

- `okind`(オプション)：Podに該当するアプリケーションをグループ化します。デプロイメント(Deployment) 名に設定すると、該当するPodを一つにグループ化します。
