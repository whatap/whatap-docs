...
description: コンテナ内のJavaアプリケーションをモニタリングするためのエージェントのインストール段階です。
tags:
  - Kubernetes
  - Kubernetesモニタリング
  - アプリケーションインストール
...
Dockerコンテナベースで実行するJavaアプリケーションのJVM Optionにエージェント適用のための設定を追加し、コンテナイメージをパッケージングする過程です。
...
5.  次の例を参考にしてDockerfileを作成してください。
...
-   JVMオプション追加の詳細については、[次の文書](https://docs.whatap.io/java/install-agent#addjvmopt)を参考にしてください。
