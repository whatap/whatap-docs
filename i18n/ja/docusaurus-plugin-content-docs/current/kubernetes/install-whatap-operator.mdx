...
1. **WhaTap Operator をインストールします。**
    ...
    **スケジューリング/優先度オプション**
    ...
    ```yaml
      ...
      tolerations: []         # 標準の K8s Toleration リスト
    ```
    ...
    デフォルトの *values.yaml* ではなくカスタム設定で使用する場合は、次のコマンドでオペレーターをインストールします。
...
オペレーターで WhaTap エージェントをインストールする場合、CR 設定（リソース制約、tolerations、imagePullSecret）の詳細は [WhaTap CR 設定オプション](whatap-cr-configuration-options) を参照してください。
