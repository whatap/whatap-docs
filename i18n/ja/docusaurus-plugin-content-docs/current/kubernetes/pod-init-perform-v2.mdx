...
- **_30日以内に生成されたPodのみ表示_** ![アイコン](/img/ic-toggle-off.svg)トグルを活性化して![指示線2](/img/number-02.png)領域のPod一覧で30日以内のPodだけを照会できます。
  ...
  **_Pod開始分析_**メニューは、Pod生成日から直近30日以内の範囲内で5分間のPod開始データを分析します。 つまり、生成後30日が過ぎたPodの場合、開始性能を分析できません。 
...
<ImgLang img='k8s-pod-startup-analysis-30days.png' desc='30日以内のポッドのリスト' />

**_Pod開始分析_**メニューは、Pod生成日から直近30日以内の範囲内で5分間のPod開始データを分析します。 そのため、生成後30日が過ぎたPodの場合、開始性能を分析できません。 

Podリストで30日以内のPodのみ照会するには、画面の上にある<Cmdname sid="BTN07717" className="uitext" days="30" />トグルオプションを有効にします。 オプションの右側にある検索入力画面に文字を入力し、希望するPodを検索できます。
