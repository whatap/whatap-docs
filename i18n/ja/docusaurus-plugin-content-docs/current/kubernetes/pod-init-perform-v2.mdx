...
- ![更新アイコン](/img/ico-refresh.svg) **_更新_**アイコンを選択して![指示線2](/img/number-02.png)領域のPod領域を再度読み込むことができます。 

- **_30日以内に生成されたPodのみ表示_** ![アイコン](/img/ic-toggle-off.svg)トグルを活性化して![指示線2](/img/number-02.png)領域のPod一覧で30日以内のPodだけを照会することができます。

  :::note 

  **_Pod開始分析_**メニューは、Pod生成日から直近30日以内の範囲内で5分間のPod開始データを分析します。 つまり、生成後30日が過ぎたPodの場合、開始性能を分析することはできません。 

  :::

- Pod検索入力ウィンドウに文字を入力して希望するPodを検索することができます。

### Pod一覧領域

![指示線2](/img/number-02.png)領域でPod一覧を確認することができます。 Pod一覧が開いたテーブルのヘッダーカラムには、次の情報が含まれています。
...
![指示線2](/img/number-02.png)領域のPod一覧で分析を希望する項目の一番左の![詳細を見るアイコン](/img/ico-detail-view.svg) **_詳細を見る_**ボタンを選択してください。 選択したPodのコンテナ別の開始性能を区間別に分析した内容を次のように確認することができます。
