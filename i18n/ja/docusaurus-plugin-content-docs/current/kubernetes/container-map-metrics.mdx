...
ホーム画面 > プロジェクト選択 > <Cmdname sid="dashboard" className="uitext" /> > <Cmdname sid="containerMap" className="uitext" /> > 先を選択後**詳細ビュー**画面に入ります > <Cmdname sid="metrics" className="uitext" />
...
-   **詳細を見る** 画面 > <Cmdname sid="metrics" className="uitext" /> - リソース使用量のモニタリングと分析
...
| ^           | CpuByRequest   | <span class="uitext">Container CPU Usage by Request (%)</span>          | コンテナCPUの要求比較の使用率                             |
...
| ^           | MemWsByRequest | <span class="uitext">Container Memory Working Set by Request (%)</span> | コンテナメモリの要求基準workingset使用量                    |
| ^           | MemWsByLimit   | <span class="uitext">Container Memory Working Set by Limit (%)</span>   | コンテナメモリ割当量基準working set使用量                   |

:::note

Podが実行中のノードに使用可能なリソースが十分である場合、コンテナに設定されたRequest以上のリソースを使用できます。WhaTapメトリックのうち、<span class="uitext">Container CPU Usage by Request (%)</span>、<span class="uitext">Container Memory Working Set by Request (%)</span>の値が100%を超える可能性がある理由です。

例えば、コンテナのMemory Requestを128MiBに設定したとします。コンテナ(Pod)が実行中のNodeの合計メモリは4GiBであり、そのNodeに他のPodがない場合、コンテナは128MiB以上のメモリを使用できます。
