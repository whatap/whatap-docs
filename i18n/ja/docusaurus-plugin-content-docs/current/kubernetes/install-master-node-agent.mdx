...
ユーザーのKubernetesバージョンとContainer RuntimeのYAMLファイルをダウンロードしてエージェントをインストールする手順です。
...
2.  **_エージェントインストール_**画面で**VERSION**と**CONTAINER-RUNTIME**のYAMLファイルを選択し**_ダウンロード_**ボタンを選択してください。
