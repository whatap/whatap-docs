...
WhaTap Kubernetesの***Pod起動分析***機能を活用すると、Pod起動時の最大使用量と初期化時間を分析し、最適なInitial Delay(Livenessチェック除外時間)設定に効果的です。
...
- ![number 01](/img/number-01.png)領域の右上にある ![更新アイコン](/img/ico-refresh.svg)***更新***アイコンを選択して更新できます。

- ![number 01](/img/number-01.png)領域の左側にある ![カラム選択アイコン](/img/ic-k8s-filter.svg)***カラム選択***アイコンを選択してカラムを選択できます。 
...
***Pod起動分析***のリストは次のとおり、<Cmdname sid="pod_info" className="uitext" />とPod初期化のタイムラインによりカラムを分類します。
