...
ホーム画面&gt; クラスタープロジェクト選択(<code class='my'>CP</code>) &gt; <Cmdname sid="side_cluster" className="uitext" /> &gt; <Cmdname sid="side_etcdDashboard" className="uitext" />
...
{/* •	etcd 리소스 사용량
•	CPU及びメモリ使用量、ファッド再起動数、関連ログデータの提供
•	作業処理状態
•	etcdのcommit及びapply回数
•	リーダー状態の追跡
•	etcdリーダーの存在有無とリーダーの変更履歴 */}
