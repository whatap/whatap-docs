...
<summary>スタンドアロン YAML でインストールする場合</summary>
...
1. 初めてエージェントをインストールする場合は、次のコマンドを実行してエージェントアプリケーションをインストールしてください。*values.yaml* は以下の例を参考にしてください。

    <Tabs groupId="event-reception">
    <TabItem value="frist install" label="初回インストール">
    ...
    <TabItem value="user" label="既存ユーザー">

    ```yaml
    #既存の WhaTap Kubernetes エージェント利用者は、次のコマンドを実行してエージェントをインストールしてください。
    #既存の YAML ファイル方式で WhaTap Kubernetes エージェントをインストールした場合、Clean Install が必要です。 既存の Kubernetes エージェントを削除した後、Helm チャートを使用してインストールしてください。.
...
OpenAgent は、認証が必要なエンドポイントをスクレイピングするために HTTP Basic Authentication をサポートしています。
ユーザー名（username）とパスワード（password）は、Kubernetes Secret を使用して安全に管理されます。
...
- `__meta_kubernetes_pod_phase`: Podの状態 (`Running`, `Pending` 등)
