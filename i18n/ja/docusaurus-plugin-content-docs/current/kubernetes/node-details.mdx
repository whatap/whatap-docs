---
id: node-details
title: Node詳細情報
description: ノードの詳細情報を確認できます。
tags:
  - Kubernetes
  - Kubernetesモニタリング
  - クラスター
  - Node詳細情報
---

ホーム画面 > プロジェクト選択 > <Cmdname sid="cluster" className="uitext" /> > <Cmdname sid="node_detail" className="uitext" />

***ノード詳細***画面は、上部の4つのチャートとノード内の***コンテナ***及び***Pod***詳細情報で構成されています。 ノードセレクタを通じて、照会を希望するノードを選択できます。

<ImgLang img='k8s-resource-node-detail.png' desc='ノード詳細画面' />

:::note 

5秒周期で最近の1分のデータを更新します。

:::

### トップチャート

次の4つのチャート情報を確認できます。

- ***CPU Usage***: ノードのCPU使用量を確認できます。

- ***Memory Usage***: ノードのメモリ使用量を確認できます。

- ***Network***: ノードのネットワークインターフェイスの入出力トラフィックとエラーパケットを確認できます。

- ***Disk Usage***: ノードのDisk使用量を確認することができます。 

### コンテナ及びPodの詳細情報

#### コンテナ

***コンテナ***タブの選択時、ノード内のコンテナ詳細情報の上部で、`cpu_quota`、`cpu_request`、`cpu_total_milli`、`mem_limit`、`mem_request`、`mem_working_set`メトリクスの合計を確認できます。 

- ***コンテナ***タブの選択時、ノード内のコンテナ詳細情報の上段で`cpu_quota`、`cpu_request`、`cpu_total_milli`、`mem_limit`、`mem_request`、`mem_working_set`指標の合計を確認できます。 

- コンテナ選択時に***コンテナ一覧***メニューに移動します。 詳細については、[次の文書](resource-container-list)を参照してください。

{/* 컨테이너 상세 정보는 다음 내용을 제공합니다. 

| 項目               | 説明                                                |      |
| ---------------- | ------------------------------------------------- | ---- |
| 状態               | コンテナ状態                                            |      |
| コンテナ             | Container ID                                      |      |
| イメージ             | コンテナイメージ情報                                        |      |
| コマンド             | コンテナ実行コマンド                                        |      |
| アプリケーション         | コンテナにインストールされているWhaTap APMエージェントの名前               |      |
| Pod              | Pod名                                              |      |
| CpuByLimit       | CPU Limit基準CPUの全体使用率                              |      |
| MemoryWsByLimit  | コンテナメモリのLimit基準working set使用量                     |      |
| CpuThrottledCnt  | コンテナCPU Throttled数                                |      |
| CpuThrottledTime | コンテナCPU Throttled時間                               |      |
| MemLimit         | コンテナのメモリLimitサイズ                                  |      |
| MemRequest       | コンテナのメモリRequestサイズ                                |      |
| MemWs            | コンテナのメモリworking set                               |      |
| IoReadBytes      | コンテナ全体ブロックデバイスの1秒当たり読み込んだバイトの合計                   |      |
| IoReadIops       | コンテナ全体ブロックデバイスの1秒当たり読み込んだ数の合計                     |      |
| IoWriteBytes     | コンテナ全体ブロックデバイスの1秒当たり書き込んだバイトの合計                   |      |
| IoWriteIops      | コンテナ全体ブロックデバイスの1秒当たり書き込んだ数の合計                     |      |
| NetRxBytes       | コンテナ全体ブロックデバイスの1秒当たり読み取りバイトの合計                    |      |
| NetRxDropped     | コンテナネットワーク受信dropped数                              |      |
| NetRxError       | コンテナのネットワーク受信エラー数                                 |      |
| NetRxIops        | コンテナのネットワーク受信数                                    |      |
| NetTxBytes       | コンテナのネットワーク送信データサイズ                               |      |
| NetTxDropped     | コンテナのネットワーク送信dropped数                             |      |
| NetTxError       | コンテナのネットワーク送信エラー数                                 |      |
| NetTxIops        | コンテナのネットワーク送信数                                    | */} |

#### Pod

***Pod***タブの選択時、Pod詳細情報を照会します。 ***コンテナ***タブの選択時、ノード内のコンテナ詳細情報の上部で、`cpu_quota`、`cpu_request`、`cpu_total_milli`、`mem_limit`、`mem_request`、`mem_working_set`メトリクスの合計を確認できます。 

- 希望するカラムを選択すると、そのカラムを基準にPod一覧をソートして照会することができます。 

- Pod選択時に***Pod一覧***メニューに移動します。 詳細については、[次の文書](resource-pod-list)を参照してください。 

{/* Pod 상세 정보는 다음 내용을 제공합니다. 

| 項目            | 説明                             |      |
| ------------- | ------------------------------ | ---- |
| Pod           | Podの固有の名前                      |      |
| PodIp         | Pod固有のIPアドレス                   |      |
| Namespace     | Podの属するネームスペース                 |      |
| Node          | Podが実行中のノード名                   |      |
| ConRestartCnt | コンテナの再起動回数                     |      |
| Age           | Podが実行された時間                    |      |
| CpuTotUsage   | PodのCPU使用量の合計                  |      |
| CpuRequest    | Podが要請したCPUリソース                | */} |
