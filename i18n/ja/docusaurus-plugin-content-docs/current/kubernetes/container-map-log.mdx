...
### Standard Out保存のためのノードエージェント設定の追加

WhaTapログサービスを通じてコンテナのStandard Outputを保存するためには、WhaTap Kubernetesノードエージェントに`logsink_enabled=true`設定を追加する必要があります。WhaTap KubernetesノードエージェントはDaemonSet形式です。

<!-- 개발자에게 관련 사항 문의 필요 -->

-   WhaTapモニタリングサービスページで**_管理_** > **_エージェント設定_**を通じて、`logsink_enabled=true`設定を追加
-   クラスタープロジェクトの全体ノードを対象に設定を追加するには、**_プロジェクトデフォルト_** > **_ノード_**を対象に設定を追加
-   一部のノードを対象に設定を追加するには、単一または複数のノードを選択した後に設定を追加

**_管理_** > **_エージェント設定_** > ノードエージェント選択 > **_+ オプション追加_** > すべてのエージェントに適用するオプションを選択 > 適用対象ノード選択 
