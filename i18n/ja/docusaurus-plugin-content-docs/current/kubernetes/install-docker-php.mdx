...
エージェントが正常にインストールされたかを確認するには、***ダッシュボード*** > ***アプリケーションサービスダッシュボード***メニューへ移動してください。
...
-   該当コンテナ内の*/usr/whatap/php/logs*経路の内容を確認してください。 エージェントログは、*logs/whatap-`{boot、またはinstall}`-yyyymmdd.log*形式のファイル名として出力されます。