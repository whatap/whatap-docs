...
次は、CallerまたはCalleeを持つすべてのトランザクションの統計です。
...
**_Caller Callee概要_**タブは、次のようにURLのCaller、Callee間の依存関係情報を表示します。この時、最大0.004、0.003など最大値が1の数字情報を表示しますが、これは全体のCallerの中で占める割合を表示します。同様に各Calleeの割合が右側にまとめて表示されます。
