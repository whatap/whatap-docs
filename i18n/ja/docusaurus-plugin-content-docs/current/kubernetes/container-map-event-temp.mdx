...
リソースライフサイクルの間、発生するすべてのイベントに対する記録を提供します。 WhaTap Kubernetesモニタリングで提供するイベント情報は次のとおりです。
...
-   ![number 1](/img/number-01.png)クバネティスイベント(**_Kubernetes_**)または、WhaTapイベント(**_WhaTap_**)を選択して、照会することができます。 
    ...
    -   クラスタリングまたは個別ブロック選択
        ...
        **_コンテナマップ_**上段の**_２段階_**グループ化選択または、個別ブロック選択すると、該当イベントを照会することができます。 **グループ化**(クラスタリング)に関する詳細内容は[次の文書](container-group)を参照してください。
...
-   ![number 2](/img/number-02.png)時間選択者を通じて、イベント記録を照会しようとする時間帯を指定することができます。 デフォルト設定は**_1日_**です。
...
-   ![number 4](/img/number-04.png)一覧形式でイベント履歴情報を確認できます。 
    ...
    -   **_Kubernetes_**イベント
        ...
        -   各イベントの左の帯もまたイベントタイプを表しています。 グレー色は<span class='k8s-normal'>Normal</span>, オレンジ色は<span class='slow'>Warning</span>状態を意味しているいます。
...
各イベントの左の帯はイベントタイプを表しています。 グレー色は<span class='k8s-normal'>Normal</span>, オレンジ色は<span class='slow'>Warning</span>状態を意味しているいます。 ユーザー選択対象によって提供されるKubernetesイベントは次のとおりです。
...
-   ![number 4](/img/number-04.png)時間選択者を通じて、照会をする時間を選択してください。 デフォルト設定は**_１日_**です。
...
-   ![number 6](/img/number-06.png)イベント履歴一覧でイベント履歴情報を確認してください。 一覧左の帯を通じて、イベント別レベルを確認できます。 イベントメッセージおよびタグ全体を確認するには![下向き矢印のアイコン](/img/ic-caret-down.svg)アイコンを選択してください。
