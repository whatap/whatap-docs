...
WhaTap Kubernetesエージェント1.5.5バージョン以降が必要です。 1.5.5バージョン未満の場合は、既存の***マスターメタ情報***画面が提供されます。
...
オブジェクトの種類の一覧からオブジェクトを選択すると、そのオブジェクトの一覧を確認できます。 ***Name***、***Namespace***、***Uid***、***CreatedTime***などのオブジェクト情報が表示されます。 次の画面は、オブジェクト種類の一覧から***Deployment***を選択する時の例です。

<ImgLang img='k8s-object-manifest.png' desc='Object Manifest' />

### Object Manifest情報の詳細

オブジェクト一覧で![アイコン](/img/ico-detail-view.svg) ***詳細を見る***ボタンの選択時***オブジェクトマニフェスト詳細***ウィンドウで選択したオブジェクトのマニフェストを照会できます。
...
***オブジェクトマニフェスト詳細***画面でオブジェクトマニフェスト内の特定のキーワードを見つけるための検索機能を提供します。 検索バーに入力した文字列は、オブジェクトマニフェスト内でハイライトされます。 これにより、複雑なマニフェスト構造で特定の設定やフィールドをすばやく確認できます。 

また***オブジェクトマニフェスト詳細***画面の右側に、マニフェスト全体の構造をツリー形式で要約したナビゲーションパネルを提供します。 これにより、大容量マニフェスト文書を扱う際に、別途のスクロールなしに主要項目を確認できます。 

#### Kubernetesイベント連携

***オブジェクトマニフェスト詳細***画面で<code class='pay'>Warning ![アイコン](/img/ic-popup-wh.svg)</code>ボタンをクリックすると、次のようにKubernetesイベントを確認できます。 
...
![アイコン](/img/ic-toggle-on.svg) ***比較する***トグル活性化時に保存時間リストから選択し、次のように2つのオブジェクトマニフェストを比較できます。 変更事項がある場合、ハイライトされます。 
...
***コンテナ·マップ***で***オブジェクトマニフェスト***を確認できます。 

<ImgLang img='k8s-object-manifest-cm.png' desc='コンテナマップからObject Manifest照会' />

- ブロックを選択します。 

- 要約情報の画面で***対象情報***タブを選択し、Podまたはコンテナ情報を確認します。  

- ***Pod***、***Deployment***、***ReplicaSet***、***Node***タグの右の![詳細を見るアイコン](/img/ico-more.svg)***詳細を見る***アイコンをクリックして***オブジェクトマニフェスト***を選択します。
