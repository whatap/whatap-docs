...
ホーム画面 > プロジェクト選択 > **_アプリケーション_** > <Cmdname sid="dashboard" className="uitext" />> <Cmdname sid="serviceDashboard" className="uitext" /> 
...
- **_ダッシュボード_**メニューまたは**_アプリケーション_**メニュー下位の**_ダッシュボード_**メニューからアクセスできます。

:::

<ImgLang img='k8s-ap-service-dashboard.png' desc='APPサービスダッシュボード'/>
...
1. 画面左上の時間選択から![ストップアイコン](/img/ico-pause.svg)ボタンを選択します。
...
ユーザーが設定した時間に基づいて、ダッシュボードに配置したウィジェットのデータを更新します。 リアルタイムモードに切り替えるには![再生アイコン](/img/ico-play-time.svg)ボタンを選択します。
...
エージェントを1つ以上選択した状態で再度すべてのエージェントを選択するには、選択を解除するか、**_Total_**を選択します。

:::

:::note

プロジェクトに接続されたエージェントの数が多い場合は、エージェント名を短く設定することをお勧めします。 エージェント名の設定については、[次の文書](../java/agent-name)を参照にしてください。
...
Javaプラットフォームでは、<Cmdname sid="side_correlatedProjectManagement" className="uitext" />メニューでデータベースプロジェクトを追加すると、次のウィジェットで連携したプロジェクトの性能指標を照会できます。
...
- 分割されたアーク領域を選択すると、該当エージェントが監視するアプリケーションから進行中のトランザクション一覧がポップアップ画面に表示されます。 詳細なトランザクション情報を確認するには、トランザクション一覧から任意の項目を1つ選択します。 
  ...
  - トランザクション一覧で選択した項目が既に終了しているトランザクションの場合は、<Cmdname sid="show_trace" className="uitext" />ボタンを選択します。 <Cmdname sid="TTL06527" className="uitext" />画面が表示されます。 <Cmdname sid="TTL06527" className="uitext" />画面の詳細については<Link to="../java/trs-profile">次の文書</Link>を参照してください。

  - トランザクションの詳細については、[次の文書](../java/track-transactions-intro)を参照にしてください。
...
- エージェント設定により、ポリシーを設定できます。 詳細については、[次の文書](../java/agent-transaction)を参照にしてください。
...
コンテナCPU使用量です。 リアルタイムでCPU使用量の変化の推移を把握できます。
...
- JavaとNode.jsでは、ヒープメモリの合計量と使用量が表示されます。

- PHPおよびPython, .Netでは、プロセスのメモリの使用量が表示されます。

:::

:::tip

ヒープメモリおよびヒープメモリチャートの分析については、次のリンクを参照にしてください。

- [学習する: ヒープメモリ メトリクスおよびメトリクス通知設定](https://docs.whatap.io/ja/best-practice-guides/about-apm-heap-memory)
...
- ユーザー数の集計のためのエージェントオプションの設定方法は[次の文書](../java/agent-number-of-user)を参照してください。
