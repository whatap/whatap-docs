...
- コンテナ内の_whatap.conf_ファイルを修正して設定できます。 

- _whatap.conf_ファイル内のオプションがコンテナ`env`フィールドに設定されたオプションよりも高い優先順位を持ちます。 

- _whatap.conf_またはコンテナ`env`に設定された値がない場合は、デフォルト値が使用されます。

:::note 

**コンテナ内のアプリケーションエージェントのオプション設定**

コンテナ内のJava、Python、Node.jsなどのアプリケーションをモニタリングするために必要な設定を_whatap.conf_ファイルに作成します。 _whatap.conf_ファイルを使用して設定できるオプションの詳細については、次のリンクを参照してください。
...
- **inject_container_id_to_apm_agent_enabled** <span class='type'>bool</span>
  ...
  WhaTap APMコンテナに`exec`を利用したcontainer IDの注入を活性化します。 WhaTap APMで識別されるコンテナのIDを自動的に抽出します。 これをAPMツールに注入し、性能モニタリングデータとコンテナを接続することができます。
...
- **whatap.server.host** <span class='type'>string</span>
  ...
  デフォルト値`13.124.11.223`あるいは`13.209.172.35`
...
- **ONODE_NAME** <span class='type'>string</span>
  ...
  デフォルト値`NODE_NAME` or `NODE_IP`環境変数値
...
- **count_interval** <span class='type'>number</span>
  ...
  メトリクス(`kube_pod`、`container`)を収集サーバーに送信するサイクルを設定します。 デフォルト値の単位はmsです。
...
- **cycle** <span class='type'>number</span>
  ...
  control-plane-helperが指標をスクレイピングするサイクルを設定します。 デフォルト値の単位はsです。 
...
- **master_to_control_plane_connection_timeout** <span class='type'>number</span>
  ...
  whatap-master-agentがwhatap-control-plane-helperとの通信時、connection timeoutが発生するまでの時間を設定します。 デフォルト値の単位はmsです。 

- **master_to_control_plane_read_timeout** <span class='type'>number</span>

  既定値`20000`

  whatap-master-agentがwhatap-control-plane-helperとの通信時、read connection timeoutが発生するまでの時間を設定します。 デフォルト値の単位はmsです。
...
- **store_k8s_object_manifest_timezone** <span class='type'>string</span>
  ...
  > 例、韓国時間基準で午前0時に実行したい場合`GMT+9`あるいは`Asia/Seoul`のようにタイムゾーン設定

- **store_k8s_object_manifest_max_fetch_limit** <span class='type'>number</span>

  デフォルト値`50`

  一度に収集するオブジェクトの照会数を設定します。 別途設定がない場合、デフォルト値の`50`を基準にして1回のリクエストにつき50個ずつリクエストします。
