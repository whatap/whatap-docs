...
- **collect_kube_node_process_metric_target_list** <span class='type'>list</span> <ScrollToLink anchor='collect_kube_node_process_metric_target_list' />
  ...
  **WhaTapでデフォルト設定されたモニタリング対象プロセスのリスト**
  ...
  - **aws-k8s-agent** 
    ...
    {/* 이 프로세스는 포드 간의 네트워크 트래픽을 처리하기 위해 Elastic Network Interface(ENI)를 동적으로 할당 및 해제하는 작업을 수행합니다. ネットワーク接続障害の時、関連する問題をデバッグするのに重要な役割を果たします。 */}
...
- **containerd-shim** 
  ...
  `containerd`とコンテナランタイムの間で中間インタフェースとして機能するプロセスです。 このプロセスは、各コンテナーのライフサイクルを管理し、コンテナーの標準入出力処理およびログ収集を担当します。
...
- **dockerd** 
  ...
  Dockerデーモンであり、コンテナイメージのビルド、展開、および実行を統括するコアプロセスです。 コンテナー間のネットワーク管理やデータボリューム管理を含む様々な機能を提供します。
...
- **aws-k8s-agent** 
  ...
  AWS環境でKubernetesネットワークインターフェース(VPC CNI)を管理するエージェントです。 このプロセスは、フォード間のネットワークトラフィックを処理するために、Elastic Network Interface(ENI)を動的に割り当ておよび解除する作業を実行します。 ネットワーク接続障害の時、関連する問題をデバッグするのに重要な役割を果たします。

- **kube-proxy** 

  クラスター内でフォード間のネットワークトラフィックをルーティングし、サービスとフォード間の通信を中継するプロセスです。 Linuxの`iptables`またはIPVSを使用してクラスター内のネットワーク要請を正しいフォードに伝達し、サービスディスカバリーとロードバランシングをサポートします。

  :::

- **kubelet:**\
  Kubernetesマスターと通信し、ノードでコンテナの状態を管理し、正しく配布および実行されるようにサポートします。

- **containerd-shim:**\
  containerdとコンテナランタイム間のインタフェースで、コンテナーのライフサイクルとログを処理します。

- **containerd:**\
  コンテナーの実行、画像管理、ネットワーキングなどを担当するコンテナランタイムです。

- **dockerd:**\
  Dockerコンテナー管理の核心プロセスであり、コンテナイメージのビルド、実行、配布、ネットワークおよびデータ管理などを行います。

- **crio:**\
  Kubernetes環境でコンテナーを管理するコンテナランタイムです。

- **coredns:**\
  Kubernetesクラスター内でDNSサービスを提供し、ネームリゾリューションおよびサービスディスカバリー、ロードバランシングをサポートします。

- **aws-k8s-agent:**\
  AWS環境でKubernetes VPC CNIを管理し、フォード間通信のためのAWSネットワークリソースの割り当ておよび解除を実行します。

- **kube-proxy:**\
  クラスター内のネットワークトラフィックをルーティングし、サービスディスカバリーとロードバランシングをiptables または IPVS を通じてサポートします。 
...
| ------------------------------------------ | ---------------- | ------ | ---------------------------------------------------------- | --------------------------------------------------------------------------- |
| `collect_volume_detail_enabled`            | `true`           | bool   | ノードエージェントですべてのコンテナに対する`exec df`コマンドを使用してコンテナボリューム情報の照会を活性化 | ノードに実行中のすべてのコンテナーのストレージ使用量とボリューム情報を収集する機能を活性化します。                           |
