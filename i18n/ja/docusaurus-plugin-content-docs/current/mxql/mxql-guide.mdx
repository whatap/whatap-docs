...
MXQLで照会するデータは[メトリクス](../metrics/common-metrics-intro)の構造に応じて異なるロードコマンドを使用します。

- **タグベースメトリクス**  
  タグとフィールドに区分されたデータは[`TAGLOAD`](mxql-loading#tagload)コマンドを使用してロードします。

- **フラット構造メトリクス**  
  すべてのデータがフィールドに保存されたメトリクスは[`FLEXLOAD`](mxql-loading#flexload)コマンドを使用してロードします。

- **ログ元データ**  
  ログ元データを照会する際は[`LogSink`](mxql-loading#logsink)コマンドを使用します。

- **ログカウント集計データ**  
  時間単位でログ件数を集計する際は[`LogSinkCount`](mxql-loading#logsinkcount)コマンドを使用します。
