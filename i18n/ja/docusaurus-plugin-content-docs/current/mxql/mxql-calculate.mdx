...
| okind  | `okid`カラム名を設定して、`okid`値に対応する`okind name`値のカラムを作成します。  |
| onode  | `onode`カラム名を設定して、`onid`値に対応する`onode name`値のカラムを作成します。 |
...
**表1**と**表2**はクエリの結果を表わし、`pk`として設定した`file_name_4`フィールドは青で表示します。**表3**は`pk`に設定した`file_name_4`を基準に2つのクエリの組み合わせ結果を示します。  
...
抽出するデータの数を制限します。その前に設定したデータ数を次のステップに渡します。
...
フロント側で`INJECT`コマンドのオペランドにマッピングされる情報を伝達する必要があります。次の例では、キーが`default`に設定された値を追加します。
