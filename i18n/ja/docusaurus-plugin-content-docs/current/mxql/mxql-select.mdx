...
`OKIND`、`ONODE`コマンドの場合は、`CATEGORY`コマンドで設定したカテゴリに`okind`、`onode`関連フィールド(`okind`、`onode`、`okindName`、`onodeName`)が含まれている場合に使用できます。***サイトマップ***　> ***分析*** > ***メトリクス参照***メニューで、どのカテゴリがどのフィールドが含まれているか確認できます。 
