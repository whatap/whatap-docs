...
:::note
`OKIND`、`ONODE`コマンドの場合は、`CATEGORY`コマンドで設定したカテゴリに`okind`、`onode`関連フィールド（`okind`、`onode`、`okindName`、`onodeName`）を含める場合に使用できます。**_サイトマップ_**　> ***分析*** > **_メトリクス照会_**メニューでどのカテゴリにどのフィールドが含まれているか確認できます。 
:::
...
-   `etime以前の15秒間のデータを照会(パラメータとして`etime\`を渡す)
...
-   `stime以前の15秒間のデータを照会(パラメータとして`stime\`を渡す)
