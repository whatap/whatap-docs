...
次は`db_mysql_counter`カテゴリに収集されるタグに対する案内です。
...
次は`db_mysql_counter`カテゴリに収集されるフィールドに対する案内です。

| Metrics                                           | Description                                         |
| ------------------------------------------------- | --------------------------------------------------- |
| `active_sessions`                                 | 現在、活性化されているセッションの数です。                               |
| `connect_error`                                   | 接続エラーの回数です。                                         |
| `cpu`                                             | SQL Serverインスタンスが使用するCPUの比率です。                      |
| `cpu_cores`                                       | 使用可能なCPUのコア数です。                                     |
...
| `total_elapsed_time`                              | SQL Serverで実行されたすべてのリクエストの経過時間です。                   |
...
| `Buffer_cache_hit_ratio`                          | データのリクエスト時にバッファキャッシュから読み取ったデータの割合を示します。             |
...
| `Errors/sec`                                      | 1秒あたりに発生したエラーの数です。                                  |
| `FreeSpace_Scans/sec`                             | 1秒あたりにフリースペースをスキャンした回数です。                           |
...
| `Lazy_writes/sec`                                 | 1秒当たりにディスクに書き込まれた遅延したページの数です。                       |
| `Lock_Requests/sec`                               | 1秒当たりのロックリクエストの数です。                                 |
| `Log_Flushes/sec`                                 | 1秒あたりのログフラッシの回数です。                                  |
| `MEMORY_ALLOCATION_EXT(count)`                    | メモリ割り当ての拡張回数です。                                     |
| `MEMORY_ALLOCATION_EXT(time)`                     | メモリ割り当ての拡張にかかった合計時間です。                              |
| `Memory_Grants_Outstanding`                       | メモリ割り当ての待機が完了したリクエストの数です。                           |
| `Memory_Grants_Pending`                           | メモリ割り当てを待機しているリクエストの数です。                            |
| `Number_of_Deadlocks/sec`                         | 1秒当りに発生したデッドロック状態の数です。                              |
...
| `PAGEIOLATCH_UP(time)`                            | I/O作業時に、アップデートラッチのリクエストにかかった合計時間です。                 |
...
| `PREEMPTIVE_OS_REVERTTOSELF(count)`               | 自身に再び切り替える作業のプリエンプティブ運用リクエストの回数です。                  |
| `PREEMPTIVE_OS_REVERTTOSELF(time)`                | 自身に再び切り替える作業にかかった合計時間です。                            |
...
| `Page_reads/sec`                                  | 1秒当たりのディスクから読み取ったページ数です。                            |
| `Page_writes/sec`                                 | 1秒当たりのディスクに書かれたページ数です。                              |
| `RESERVED_MEMORY_ALLOCATION_EXT(count)`           | 予約されたメモリ割り当て拡張の回数です。                                |
| `RESERVED_MEMORY_ALLOCATION_EXT(time)`            | 予約されたメモリ割り当て拡張にかかった合計時間です。                          |
| `Range_Scans/sec`                                 | 1秒当たりの範囲スキャンの回数です。                                  |
| `SOS_SCHEDULER_YIELD(count)`                      | スケジューラが作業を処理するために譲った回数です。                           |
| `SOS_SCHEDULER_YIELD(time)`                       | スケジューラが作業を処理するために譲った合計時間です。                         |
...
| `Stolen_Server_Memory_(KB)`                       | SQL Serverが使用するために持っていったメモリのサイズ(KB)です。              |
| `Total_Latch_Wait_Time_(ms)`                      | ラッチ待機による待機時間の合計(ミリ秒単位)です。                           |
| `Total_Server_Memory_(KB)`                        | SQL Serverが使用しているメモリサイズの合計(KB)です。                   |
| `Transactions/sec`                                | 1秒当たりのトランザクション数です。                                  |
| `User_Connections`                                | SQL Serverに接続されているユーザーセッションの数です。                    |
