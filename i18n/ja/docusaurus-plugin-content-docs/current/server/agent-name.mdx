...
WhaTapは、モニタリング情報の収集対象となるインフラストラクチャサーバーの識別のために、サーバーから収集した情報を使用します。 デフォルト値は、サーバーのホスト名(hostname)です。

{@include: ../common-items/_agent-name-warning.mdx}

:::note

ユーザーが設定したエージェント名を [WhaTapのモニタリングサービス](https://service.whatap.io)を確認するには、サーバーを再起動してください。
