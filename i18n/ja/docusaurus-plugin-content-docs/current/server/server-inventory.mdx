...
<Cmdname sid="side_serverInventory" className="uitext" />メニューでサーバー構成データを自動的に収集して管理できます。 また、サーバー構成項目をユーザーが定義して追加または修正できます。 これにより、サーバーの運営および管理に必要な重要なデータを一目で把握し、追加の構成管理項目を通じてサーバー管理の効率性を向上できます。
...
フィルター入力後<Cmdname sid="BTN06223" className="uitext" />ボタンをクリックすると、![指示線2](/img/number-02.png)サーバー インベントリ一覧領域でフィルタリングされた一覧を照会できます。 
