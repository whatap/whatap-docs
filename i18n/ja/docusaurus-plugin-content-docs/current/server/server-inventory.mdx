...
<TabItem value="linux-restart" label="エージェントの再起動">
...
<TabItem value="unix-restart" label="エージェントの再起動">
...
フィルター入力欄をクリックし、目的の **フィルターキー** と **条件** を選択して値を入力し、**適用** ボタンをクリックしてください。サーバーインベントリ一覧領域でフィルタリングされた一覧を確認できます。

<ServerInventoryFilter />

### ソート

ソートオプションによって最大3段階のマルチソートが可能です。 カスタムカラムを含め、希望する項目を基準にソートできます。 デフォルトのソート設定は<Cmdname sid="TTL08087" className="uitext" /> &gt; <Cmdname sid="TTL08099" className="uitext" /> &gt; <Cmdname sid="TTL08100" className="uitext" />です。 
...
サーバーインベントリの過去データ照会、変更履歴照会、日別比較を通じてサーバーの変化過程を追跡できます。

#### 過去データ照会

毎日午前0時基準で保存されたスナップショットデータを呼び出し、過去データや特定フィールドの変更履歴を確認できます。

1. YYYY/MM/DD ![カレンダーアイコン](/img/ic-start-date.svg) ボタンをクリックします。

1. 照会する過去の日付を選択し、**確認** ボタンをクリックして照会します。

:::caution

過去データ照会時、**修正**、**CSVアップロード**、**カラム設定** 機能は使用できません。  
テーブルの Connection フィールドは自動的に非表示になります。

:::

#### 変更履歴照会

選択したサーバーの特定フィールド値が照会区間の間、日ごとにどのように変更されたかを確認でき、そのフィールドの生成および削除履歴もあわせて照会できます。

1. <Cmdname sid="side_serverInventory" className="uitext" />で ![時間アイコン](/img/ico-timing.svg) **変更履歴照会** ボタンをクリックします。 

1. **インベントリ変更履歴検索** ウィンドウで照会する日付、サーバー、カラム名を選択します。 

1. 選択した条件に従って日別の変更履歴を確認します。

#### 日別比較

選択したサーバーの2つの日付を指定し、データ変更内容やフィールドの生成・削除有無を比較できます。

1. 比較するサーバーの **History** ![時間アイコン](/img/ico-timing.svg) アイコンをクリックします。 

1. **ヒストリー照会**で **基準日** と **比較日** を選択してデータを比較します。

### カラム設定

<Cmdname sid="column_settings" className="uitext" /> ボタンをクリックしてください。 次のようにカラムを選択してカスタムカラムを追加した後、下にある<Cmdname sid="BTN06053" className="uitext" />ボタンをクリックしてカラムの設定を保存します。 
...
- **カラム追加** 
  ...
  ユーザーは **+** <Cmdname sid="add_a_column" className="uitext" /> ボタンを選択した後、基本提供項目に加えて、自分が望む項目を追加してカスタムカラム(<code class='custom-cl'>custom</code>)を構成できます。 必要に応じてカラムを追加し、サーバーの状態とパフォーマンスをより細かくモニタリングしてみてください。 

### CSVダウンロードとアップロード

#### CSVダウンロード{#csvdownload}

![ダウンロードアイコン](/img/ico-download.svg) <Cmdname sid="download" className="uitext" /> ボタンを選択して、収集されたサーバーインベントリを *CSV* ファイル形式でダウンロードできます。
...
1. ![アップロードアイコン](/img/ico-upload.svg) **CSV** <Cmdname sid="upload" className="uitext" /> ボタンをクリックし、アップロードする *CSV* ファイルを選択してください。
...
![修正アイコン](/img/ic-edit.svg) <Cmdname sid="edit" className="uitext" /> ボタンを選択して、サーバーインベントリ一覧を修正できます。
