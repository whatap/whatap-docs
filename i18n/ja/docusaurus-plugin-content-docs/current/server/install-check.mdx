...
id: install-check
title: インストールチェックリスト
description: サーバーモニタリングのインストールをすべて完了しました。 インストール後の確認事項をチェックしてください。
tags:
  - Server
  - エージェント
---
...
サーバーモニタリングのインストールをすべて完了しました。 インストール後の確認事項をチェックしてください。

## Linux及びUnix

### ログ出力確認

エージェントが正常にインストールされると、次のようなログ出力を確認できます。 接続が正常に完了すると、connectedというメッセージが表示されます。
...
接続できない場合は、ファイアウォール設定を含めてネットワーク設定を確認してください。 ネットワーク設定が反映されると、エージェントは自動的に接続されます。

## Windows

正常にインストールが完了した場合、インストールの最終段階で次のような画面が表示されます。 エージェントが自動的にモニタリングを開始します。 
