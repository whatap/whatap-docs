...
id: server-linux
title: Linux及びUnix
description: LinuxとUnix環境でWhaTapサーバーモニタリングエージェントをインストールする方法を提供します。
tags:
  - Server
  - インストール
  - Linux
...
![サーバーモニタリングLinux Unix]（/img/server-install-lx-unx.png）

<!-- ![](/img/server-install.png) -->

1.  WhaTap **リポジトリ**を登録します。  

    ![number-s]（/img/number-01.png） **_役人_** > **_エージェントインストール_** 画面上段のOSタブで「サーバーOS」を選択してください。 ![number-s]（/img/number-02.png） **_WhaTapリポジトリの追加_** タブで該当するコマンドをコピーして入力します。
...
2.  次のコマンドを使用して**パッケージ**をインストールしてください。<br/>
    ![number-s]（/img/number-03.png） **_WhaTapサーバーモニタリングパッチ設置_** タブで該当するコマンドをコピーして入力します。
...
3.  **設定スクリプト**を実行してください。<br/>
    ![number-s]（/img/number-04.png）**_サーバーモニタリングを開始する_** タブに移動してコマンドを発行するか、下段のコマンドにプロジェクトアクセスキーとサーバーIPを追加して実行してください。
