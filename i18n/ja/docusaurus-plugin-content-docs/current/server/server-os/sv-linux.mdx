...
**_エージェントのインストール_**画面で希望するオペレーティングシステムを選択し、プロジェクトのアクセスキーを確認したら、次の案内通りにインストールを進めてください。 

1. WhaTap **リポジトリ**を登録します。  

   **_WhaTapリポジトリの追加_**タブから該当するコマンドをコピーして入力します。
...
2. 次のコマンドを使用して**パッケージ**をインストールしてください。
   ...
   **_サーバーモニタリングパッケージのインストール_** タブで適切なコマンドをコピーして入力します。
...
3. **設定スクリプト**を実行してください。
   ...
   **_設定スクリプトを実行しサーバーモニターデモンを開始してください。_** タブに移動してコマンドを発行するか、下部のコマンドにプロジェクトアクセスキーとサーバーIPを追加してください。
