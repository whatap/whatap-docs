...
1.  Linuxインストール環境に合わせて、次の内容をElastic Beanstalkの **インストールスクリプト**に追加してください。

    :::note
    「whatap.server.host」と「license」、つまり**プロジェクトアクセスキー**はユーザーに合わせて入力します。
    :::
...
2.  次の**コマンド**を修正してください。  
    ...
    インストール後、サーバーモニタリングをBeanstalkに含めてアプリケーションをデプロイすると、サーバーモニタリングの新規リストが引き続き作成されます。 

    WhaTapエージェントの内部IDが、上記のコマンドが実行されるたびに新しく発行されるためです。 今後のデプロイ時に新規一覧が追加発生しないように、次のように修正してください。
