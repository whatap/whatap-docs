...
<Cmdname sid="side_agentInstall" className="uitext" />メニューのオペレーティングシステムの選択画面で***HP-UX***を選択します。 ユーザーのHP-UXサーバーアーキテクチャに従って、上部のタブで ***IA64***または***PA-RISC***の中から選択した後、プロジェクトアクセスキーを確認します。 そして、次の案内通りにインストールを進めます。
