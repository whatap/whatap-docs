...
インストールされているエージェントを最新バージョンにアップデートするには、<Cmdname sid="side_management" className="uitext" /> > <Cmdname sid="side_agentInstall" className="uitext" />メニューに移動してください。<Cmdname sid="agent-setup-guide" className="uitext" />セクションの<Cmdname sid="server_installAgent_win_download" className="uitext" /> 案内の指示に従って、WindowsサーバーOS用のインストールファイルをダウンロードした後 _whatap_infra.exe_ ファイルを実行してください。 エージェントのアップデートした後、サービスが再起動されます。
