...
インストールされているエージェントを最新バージョンにアップデートするには、 **_管理_** > **_エージェントのインストール_** メニューに移動してください。 **_インストール案内_** セクションの **_インストールファイルのダウンロード_** 案内の指示に従って、WindowsサーバーOS用のインストールファイルをダウンロードした後 _whatap_infra.exe_ ファイルを実行してください。 エージェントのアップデートした後、サービスが再起動されます。
