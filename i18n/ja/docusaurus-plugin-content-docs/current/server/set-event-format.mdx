...
| Server Name                                       | アプリケーション名                                          | `true`   | oname                                                               |
...
| Metric Value                                      | メトリック値                                             | `true`   | メトリック値がメトリックしきい値を超えると、イベント発生条件が満たされている場合です。                         |
| Metric Threshold                                  | メトリックしきい値                                          | `true`   | メトリック値がメトリックしきい値を超えると、イベント発生条件が満たされている場合です。                         |
...
| Stateful                                          | 解決されたイベント通知                                        | `true`   | 解決されたイベント通知機能を使用している場合は「true」、使用していない場合は「false」                     |
| Repeat Count                                      | イベントの反復回数                                          | `true`   | （メトリクス通知）イベント条件がイベント反復時間のうちイベント反復回数を満足すると、イベントが発生します。               |
| Repeat Duration                                   | イベント反復時間                                           | `true`   | （メトリクス通知）イベント条件がイベント反復時間のうちイベント反復回数を満足すると、イベントが発生します。解決されたイベント通知です。 |
| IP                                                | IP                                                 | `true`   | -                                                                   |
...
| --------------- | ------------------- | -------------------------------------------- | ----------------------------- |
| 再起動             | Critical            | System Rebooted"                             | 非対応                           |
| 非受信             | Warning             | Agent No Data"                               | 非対応                           |
...
| ディスク使用量         | Warning or Critical | Disk ${mountPoint} Used > ${value} %"        | -                             |
