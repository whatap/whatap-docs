netstatコマンドが入力された場合、1分間隔で収集し<Cmdname sid="category" className="uitext" /> `server_netstat` (netstat connection 統計), `server_netstat_s` (netstat -sコマンド実行結果)にアップロードされます。
