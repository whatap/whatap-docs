...
- **アプリケーションモニタリング**
  ...
  **_適用する_**メニューの下にある固有のアプリケーション手順を確認してください。 次の説明書[Java](log-java)、[PHP](log-php)、[Node.js](log-nodejs)、[Python](log-python)、[Go](log-go)などをご参照ください。
...
7. <Cmdname sid="ITM06439" className="uitext" />として使用する<Cmdname sid="ITM06429" className="uitext" />と<Cmdname sid="ITM06430" className="uitext" />を選択するか、直接入力してください。
...
3. <Cmdname sid="ITM06255" className="uitext" />を入力してください。

4. <Cmdname sid="category" className="uitext" />選択画面からカテゴリを選択するか、直接入力します。

5. <Cmdname sid="ITM06439" className="uitext" />を選択するか、直接入力してください。
...
<Cmdname sid="TTL08814" className="uitext" />タブでログデータ内に含まれた個人情報を暗号化及びマスキング処理して機密な情報の露出を防止できます。

<Cmdname sid="TTL08814" className="uitext" />機能は、ログデータ内に含まれる個人情報を暗号化およびマスキング処理することで、機密な情報の露出を防止し、個人情報保護に関する法規を遵守するために設計されました。 この機能は、機密なデータを扱う環境でセキュリティを強化するための不可欠ツールとして提供されます。 
*/}
...
1. <Cmdname sid="TTL08814" className="uitext" />タブで<Cmdname sid="adding" className="uitext" />ボタンを選択してください。

2. 右側の<Cmdname sid="TTL08814" className="uitext" /> <Cmdname sid="add" className="uitext" />画面で<Cmdname sid="category" className="uitext" />を選択してください。 

3. 右側の<Cmdname sid="TTL08814" className="uitext" /> <Cmdname sid="add" className="uitext" />画面で暗号化およびマスキング処理する<Cmdname sid="ITM06429" className="uitext" />を入力してください。 

4. 右側の<Cmdname sid="TTL08814" className="uitext" /> <Cmdname sid="add" className="uitext" />画面で下段の<Cmdname sid="add" className="uitext" />ボタンをクリックしてその設定を追加してください。 
...
指定された検索キーの他に、業務上の機密な情報として処理すべき検索キーがあれば、追加的に暗号化の保存および非識別化の対象に指定できます。 */}

{/* :::note 

<Cmdname sid="log_parser_setting_primary" className="uitext" />タブでその時の検索キー名を<code class='filter'>検索キー.p</code>に指定した場合、デフォルトでは非識別化の対象となります。 <Cmdname sid="TTL08814" className="uitext" />タブで追加または修正時に該当の<Cmdname sid="category" className="uitext" />を選択した後、<Cmdname sid="TTL08835" className="uitext" />で確認できます。 マスキング処理を希望しない場合は解除できます。 パーサー設定中に非識別化の対象を指定する方式より<Cmdname sid="TTL08814" className="uitext" />タブの使用を勧めます。 個人情報の非識別化の詳細は[次の文書](#log-deidentification-of-personal-info)で参照してください。

::: */}

{/* 사용자가 비식별화 설정을 하지 않아도 비식별화 처리됩니다. この場合、<Cmdname sid="TTL08814" className="uitext" />タブで操作できません。 */}
...
3. <Cmdname sid="ITM08981" className="uitext" />を例の中から選択または<Cmdname sid="direct_input" className="uitext" />を選択してください。
...
4. <Cmdname sid="ITM08983" className="uitext" />トグルで個人情報が識別された場合、該当情報の置換有無を選択してください。

5. 既存のログで正規表現式パターンに識別された文字列を変更する<Cmdname sid="ITM08984" className="uitext" />を入力してください。 
...
6. 条件を入力した後、<Cmdname sid="BTN08985" className="uitext" />ボタンを選択してください。 点検中の場合はいつでも<Cmdname sid="BTN08986" className="uitext" />ボタンで点検過程を中止できます。 

7. 点検作業の完了時に、例の画像のように右側から<Cmdname sid="ITM08995" className="uitext" />を確認できます。 
...
| ----------- | -------------------- | ---------------------------------------------------------------- | --------------------- |
| 住民登録番号      | 123456-1234567       | `([0-9]{6}-[1-4]{1}[0-9]{6})`                                             | `******-*******`      |
| パスポート番号     | M12345678            | `([a-zA-Z])`                                                     | `*********`           |
| 運転免許証の番号    | 01-23-456789-00      | `(\d{2}-\d{2}-\d{6}-\d{2})`                                      | `**-**-******-**`     |
| 外国人登録番号     | 987654-1234567       | `([0-9]{6}-[0-9]{7})` | `******-*******`      |
| クレジットカードの番号 | 1234-5678-9012-3456  | `([34569][0-9]{3}[~- ][0-9]{4}[~- ][0-9]{4}[~- ][0-9]{4})`       | `****-****-****-****` |
| 電話番号        | 010-1234-5678        | `(\d{2,3}[ ,-]?\d{3,4}[ ,-]?\d{4})`                              | `***-****-****`       |
| メールアドレス     | `example@domain.com` | `(([\w!#._-])@([\w.]\.\w{2,3}))`                                 | `***@***.***`         |
| 口座番号        | 1111-04-031111       | `(\d{4}-\d{2}-\d{7})`                                            | `****-**-******`      |
| 事業者登録番号     | 111-11-01111         | `(\d{3}-\d{2}-\d{5})`                                            | `***-**-*****`        |
| 法人番号        | 111111-1111111       | `(\d{6}-\d{7})`                                                  | `******-*******`      |

{/* 이 섹션에서는 기존 데이터에 대해 치환 값을 설정하여 개인정보를 완전히 치환합니다. 設定された置換値は、元のデータと同じタイプを維持しながら、機密な情報を安全な値に変更します。 置換されたデータは復元できないため、慎重に適用する必要があります。 */}
