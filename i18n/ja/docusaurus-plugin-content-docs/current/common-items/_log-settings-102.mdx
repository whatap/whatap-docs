...
カテゴリごとのログデータの保存期間を指定できます。 **ログ数**は、当該期間中に蓄積されたログの件数を意味します。 例えば、<Cmdname sid="daily_log_count" className="uitext" />は、1日にたまったログ件数、<Cmdname sid="expected_log_count" className="uitext" />は、データ保持日に今日のログ数を掛けたログ件数を意味します。 
