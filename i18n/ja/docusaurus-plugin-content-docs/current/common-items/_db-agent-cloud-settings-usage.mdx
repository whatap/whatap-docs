...
DBXCは安定した実行、直感的なユーザー体験、そして多様なモニタリング機能を提供します。

| 特徴         | 説明                                                                             |
| ---------- | ------------------------------------------------------------------------------ |
| **安定性**    | - PIDファイルに基づく重複実行防止 <br /> - ヘルスチェックによる正常起動確認 <br /> - 5秒起動タイムアウト設定            |
| **ユーザー体験** | - プロジェクト（エージェント）ごとの一貫したカラーコーディング <br /> - リアルタイムログストリーミング <br /> - 直感的な状態表示    |
| **モニタリング** | - HTTPサーバーベースの状態API (ポート: 53200) <br /> - 構造化ログ（Zapライブラリ） <br /> - 自動ログローテーション |
