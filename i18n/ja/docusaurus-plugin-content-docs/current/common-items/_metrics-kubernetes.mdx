...
| desired_pod                  | 整数        | `metadata.ownerReferences`なしで配布されたPod数とクバネティスオブジェクト(ReplicaSet, Daemonset, StatefulSet)に定義されたDesired Pod数の和 | -                    |
| ^                            | ^         | `kubectl get pods -A`で照会したPod数と同じ                                                                           | -                    |
| nodes                        | 整数        | ノード数                                                                                                        | -                    |
