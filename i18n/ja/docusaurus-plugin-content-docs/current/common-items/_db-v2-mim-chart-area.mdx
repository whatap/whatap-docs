...
-   **_SQL Executed_**：インスタンス別の1秒あたりのSQL実行件数の合計です。 5秒単位で収集した`Questions`（全体クエリのリクエスト回数）指標値に基づいています。
...
-   **_Full Scan_**：インスタンス別テーブルをフルスキャン(full scan)した回数をリアルタイムチャートで表示します。

-   **_tmp table_**：インスタンス別コマンドの実行中にサーバーによって生成された内部一時テーブルの数をリアルタイムチャートで表示します。
