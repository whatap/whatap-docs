...
- **profile_http_parameter_enabled** <span class='type'>Boolean</span><ScrollToLink anchor='profile_http_parameter' />
  ...
  トレース履歴にhttpパラメータ情報を記録する場合は`true`に設定してください。ただし、パラメータを表示するには、パラメータに別のセキュリティキーを入力する必要があります。セキュリティキーをエージェントインストールパスの*paramkey.txt*ファイルを6桁で指定します。*paramkey.txt*が存在しない場合は、自動生成され、randomキーが設定されます。
