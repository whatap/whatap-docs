...
1. DBXエージェントがインストールされたパスの_whatap.conf_ファイルに次のオプションを設定してください。
...
2. _xos_フォルダー(`/압축 해제 폴더/xos/`)をデータベースサーバーに移動してください。

3. データベースサーバーに移動した_xos_経路の_xos.conf_ファイルに次のオプションを設定してください。
...
エージェントサーバーにある_whatap.conf_ファイルに次のオプションを設定してください。 CloudWatch特性上、60秒(`cloud_watch_interval`)ごとの呼び出しをお勧めします。
