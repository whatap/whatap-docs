...
WhaTapモニタリングサービスのメイン画面でプロジェクトを選択し<Cmdname sid="side_statistics" className="uitext" />の下位メニューを選択してください。WhaTapエージェントは、トランザクション情報やSQLなどの主要なトレース情報について統計情報を収集します。5分ごとに一覧を作成してサーバーに送信します。

- 各**統計**タブは、**合計**、**最大**および**平均時間**および**件数**による並べ替えオプションを提供し、任意の視点から上位リストを表示することができます。 

- <Cmdname sid="BTN06441" className="uitext" />ボタンを通じて既定値の他に一覧内で照会するカラムを追加または除外できます。

- 統計リストで![詳細を見るアイコン](/img/ico-detail-view.svg) <Cmdname sid="display_detail" className="uitext" />アイコンを選択すると、件数と平均時間チャートを確認することができます。エージェント間の件数とエラー件数の比較、リスポンスタイムの比較チャートを追加で提供します。そのチャートで照会を希望する時点を選択すると<Cmdname sid="side_txSearch" className="uitext" />メニューに移動します。
...
最大31日間のデータを照会できます。ただし、<Cmdname sid="side_statRemote" className="uitext" />メニューでは最大7日間のデータを照会することができます。
...
| <Cmdname sid="httpc_count" />   | HTTP Call件数                      |
| <Cmdname sid="TTL06959" /> (ms) | HTTP Call時間の合計の平均                |
| <Cmdname sid="TTL06097" />      | SQLの実行件数                         |
| <Cmdname sid="TTL06956" /> (ms) | 平均SQL実行時間                        |
| <Cmdname sid="sqlFetch" />      | SQLパッチ件数                         |
| <Cmdname sid="TTL06953" /> (ms) | SQLフェッチ時間の合計の平均                  |

:::note

<Xclude product="php">

<Cmdname sid="TTL06975" className="uitext" />は、エージェント設定(*whatap.conf*)で<code>trace_malloc_enabled=true</code>オプションを設定しないと収集できません。既定値は<code>false</code>です。

</Xclude>

<InDoc product="php">

<Cmdname sid="TTL06975" className="uitext" />は、エージェント設定(*whatap.ini*)で<code>trace_malloc_enabled=true</code>オプションを設定しないと収集できません。既定値は<code>false</code>です。
...
<Cmdname sid="TTL06918" />は、5分間、SQLを呼び出したURL(最後の呼び出しURL)を分析利用のため収集します。
...
ユーザーエージェントの文字列を分析して、エンドユーザーが使用するブラウザーの種類、OS基準で集計した統計情報を提供します。収集した統計情報に基づいて作成されたパイチャートを通じて占有比重を確認することができます。
