...
#### アクティブ HTTPC/SQL {#active-httpc-sql}

<ImgLang img='apm-dashboard-active-httpc-sql.png' desc='アクティブ HTTPC/SQL' />

**アクティブ HTTPC/SQL** ウィジェットは、アプリケーションで発生する外部呼び出し（HTTP 呼び出しおよびデータベースクエリ）のパフォーマンスをリアルタイムでモニタリングします。  
すべてのトランザクションを表示する <Cmdname sid="AA002" className="uitext" /> ウィジェットとは異なり、外部呼び出しに特化しているため、外部システムのボトルネックを即座に把握できます。2つのウィジェットを一緒に配置することで、トランザクションの増加が外部システムの遅延によるものか、内部ロジックの問題によるものかを区別でき、より効果的な多層分析が可能になります。

進行中の HTTPC/SQL を **アークイコライザー** チャートで可視化し、遅延発生状況をホスト/データベース URL ごとに確認できます。ホスト/データベース URL の数に応じてアークが分割され、5 秒ごとに現在サーバーで処理中のリクエスト数と処理時間が表示されます。5 秒間隔で検知されたリクエストは危険度に応じて直感的な色で分類されます。

* 赤 (<span class='vslow'>Very Slow</span>): 8 秒以上

* オレンジ (<span class='slow'>Slow</span>): 3 秒～8 秒

* Normal: 0 秒～3 秒
  * 紫 (<span class='purple'>HTTPC Normal</span>)
  * ミント (<span class='mint'>SQL Normal</span>)

Normal が多く表示される状況は問題ではありません。一部がオレンジや赤に変化するかどうかを観察する必要があります。障害時には進行中トランザクション数が増加し、赤の割合が高まります。一方、応答が非常に速いシステムでは TPS が高くても進行中のトランザクション数は少ない場合があります。

このウィジェットは特にマイクロサービス環境でのサービス間 HTTP 通信遅延や、複数のデータベースを使用する環境で DB パフォーマンスを個別に追跡するのに有用です。外部 API 呼び出しが多いサービスでは、API 提供者ごとの応答時間をモニタリングでき、依存関係管理に効果的です。

##### アークイコライザーチャート

ウィジェットの中核的な可視化要素で、進行中の HTTPC/SQL をホスト/データベース URL ごとに分割して表示します。全体トランザクションで問題を発見した場合、このチャートを通じて具体的な外部呼び出しのボトルネックを確認できます。

* **左上表示**: 進行中トランザクションが最も多いホスト/データベース URL を表示し、主要対象を容易に特定
* **中央数値**: アクティブ HTTPC/SQL の合計数を表示
* **TPS ベースアニメーション**: イコライザーの外周バーがスループット (TPS) に応じて 3 段階の速度で回転し、システム負荷を視覚的に表現
* **5 秒単位更新**: リアルタイムに近いモニタリングで即時対応可能
* **アーク分割**: ホスト/データベース URL ごとにアークが分割され、どの外部システムで遅延が発生しているかを一目で把握

##### ホスト/データベース URL 詳細照会

分割されたアーク領域を選択すると、該当ホスト/データベース URL で進行中の **トランザクション Top 100 リスト** ポップアップが表示されます。

* **トランザクションリスト**: 選択したホスト/データベース URL の進行中トランザクション Top 100 を表示
* **トレンドチャート**: 過去 10 分間の TPS/応答時間/エラー件数/エラー率をチャートで表示
* **トランザクション詳細**: リストからトランザクションを選択すると詳細情報を確認可能
  * すでに終了したトランザクションの場合、**トランザクション表示** ボタンを選択して **トランザクション情報** ウィンドウに移動

1. **アクティブ HTTPC Top 100** ポップアップでトランザクションリストとトレンドを確認します。

1. 詳細分析が必要なトランザクションを選択します。

    a. アークイコライザーで分析するホストまたはデータベース URL のアークをクリックします。

1. トランザクション情報ウィンドウで詳細内容を確認します。
