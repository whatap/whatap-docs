...
| `blocking_session_id`     | 現在のセッションをブロックしているセッションのIDです。                       |
| `client`                  | セッションを作成したクライアントの名前またはIPアドレスです。                    |
| `command`                 | セッションが実行中のSQLコマンドです。 (SELECT、INSERT、UPDATE、DELETE) |
...
| `logical_reads`           | セッションで発生した論理的読み込みの回数です。                            |
| `logical_reads(sigma)`    | すべてのセッションで発生した論理的読み込みの回数の合計です。                     |
...
| `reads`                   | セッションで発生した物理的読み込み回数です。                             |
| `reads(sigma)`            | すべてのセッションで発生した物理的読み込み回数の合計です。                      |
...
| `status`                  | セッションの現状です。 例：`running`、`suspended`など。             |
