...
- **iplist_interval** <span class='type'>Second</span>
  ...
  _whatap.conf_のファイルに`xos_dbip=true`オプションを設定すると`db_ip_list`のカテゴリに転送します。<Cmdname sid="side_analysis" className="uitext" /> &gt; <Cmdname sid="side_metricsSearch" className="uitext" />メニューで`db_ip_list`カテゴリを選択した後、確認することができます。
...
- **slow_query** <span class='type'>String</span>
  ...
  データベースのスロークエリを収集します。モニタリング対象DBにXOSがインストールされているものの、ファイル名を自動的に認識できない場合は、XOS _whatap.conf_ファイルに`slow_query=`と設定します。
