...
- **dbx_port** <span class='type'>String</span>
  ...
  DBXエージェントと通信するポートを設定します。
  {/* DBXエージェントは、UDPで通信します。そのポートにファイアウォールを設定した場合は、UDP通信のためにファイアウォールを解除してください。   */}
  DBXエージェントは、UDPで通信します。ファイアウォールでUDPポートがブロックされている場合は、該当ポートを開放してください。
...
- **mem_limit** <span class='type'>Int</span>
  ...
  limitの設定値より大きい値を持つプロセス情報を収集します。単位は「KB」で既定値である10240は、10MBになります。(1024 KB = 1 MB)
...
- **port_check** <span class='type'>String</span>
  ...
  接続状態をチェックするサーバーとポートを指定します。IP:Portの形式で設定し、コンマ(,)で区切って複数登録が可能です。接続ができないサーバーとポートの一覧は、`db_port_check`カテゴリに保存されます。

  ```ini title="xos.conf"
  port_check=192.168.122.113:3333,192.168.122.113:3334
  ```  

  接続ができなかったサーバーとポートの一覧を確認する場合、<Cmdname sid="side_analysis" className="uitext" /> &gt; <Cmdname sid="side_metricsSearch" className="uitext" />メニューで`db_port_check`カテゴリを選択します。

  ![port_check](/img/db-xos-conf-port-check-metrics.png)

- **process_check** <span class='type'>String</span>

  プロセスの実行状態をモニタリングできます。オプション値でプロセス名を入力します。コンマ(,)を区切り文字として使用し。複数登録することができます。

  ```ini title='xos.conf'
  process_check=pmon,smon,tnslsnr,melong,haha
  ```

  指定された文字列を含むプロセス名が検知されない場合、`db_process_check`カテゴリにその文字列の一覧を保存します。例えば、**melong**、**haha**の2つのプロセスがない場合、<Cmdname sid="side_analysis" className="uitext" /> &gt; <Cmdname sid="side_metricsSearch" className="uitext" />メニューで`db_process_check`カテゴリを選択した後、確認できます。
...
- **process_check_interval** <span class='type'>Second</span>
  ...
  `process_check`オプションに設定されたプロセスの実行状況を確認する間隔を設定します。(単位：秒)

- **iplist_interval** <span class='type'>Second</span>

  既定値`0`(off)

  DBサーバーのIP一覧を収集すす間隔を設定します。(単位：秒、3以上で設定)
...
4. エージェントを再起動します。  
5. ![아이콘](/img/ic-sitemap.svg) <Cmdname sid="sitemap" className="uitext" /> &gt; <Cmdname sid="side_metricsSearch" className="uitext" /> メニューで、`shmon` カテゴリを確認します。
