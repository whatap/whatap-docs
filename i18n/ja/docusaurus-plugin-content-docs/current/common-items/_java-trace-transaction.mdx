...
-   **mtrace_rate** <span class='type'>Percentage</span><br/>
    デフォルト値`10`<br/>最初のトランザクションが発生したときに発行するMTIDの発行比率を設定するオプションです。
...
-   **mtrace_send_url_length** <span class='type'>Int</span><br/>
    デフォルト値`80`<br/>HTTP CallerはCalleeに自分のURLを渡します。このときURLの長さを制限しますが、この長さの値を設定します。

-   **mtrace_callee_id_send_enabled** <span class='type'>Boolean</span><br/>
    デフォルト値`false`<br/>MTID追跡時、HTTPCの呼び出しとともに発行されたCallee IDをトレースに表現します。
