...
input:
  ...
  instances: # 設定したインスタンスは常に収集します。クラスター内のインスタンスをこの属性に明示すると、オートスケール有無に関係なく常に収集します。
...
| `region`                        | AWSリージョンコード<br />- 例 `ap-northeast-1` (東京リージョン)                                                                                                                                                     |
...
>>>>>>> 084cc9d7a6143e94bdbdde03cffb0d3b3a0d9914
  clusters:
    autoscale:
      enabled: false # オートスケールが有効な場合、スケーリングされたインスタンスを収集対象に追加または削除します。
      interval: 60 # 指定したクラスターのオートスケール確認間隔です。（単位: 秒）
