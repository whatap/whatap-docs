...
例）スタックで**トップライン**は`IL_STUB_ReversePInvoke `です。 
...
`whatap.util.ThreadUtil.sleep` ← `jdbc.Control.exec` ← `jdbc.FakePreparedStatement.executeQuery`の呼び出し比率が 40.02% \* 68.06% を意味するものではありません。`jdbc.Control.exec`で他モジュールの呼び出し可能性が存在するからです。
