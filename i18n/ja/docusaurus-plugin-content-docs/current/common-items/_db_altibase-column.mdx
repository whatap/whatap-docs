...
| 項目                                   | 説明                                          |
| ------------------------------------ | ------------------------------------------- |
| `アクション`                              | 現在のセッションで実行されているアクションまたはコマンドです。             |
| `active_flag`                        | セッションの活性化可否を示します。                           |
| `autocommit_flag`                    | 自動コミットの活性化可否を示します。                          |
...
| `current_stmt_id`                    | 現在実行中のSQL文のIDです。                            |
| `data page gets`                     | データページを読み込むためにアクセスした回数です。                   |
| `data page read`                     | データページを物理的に読み込んだ回数です。                       |
...
| `failover_source`                    | フェイルオーバー時の元のサーバーの情報です。                      |
...
| `max_statements_per_session`         | セッション当たりの最大実行可能な文章数です。                      |
| `memory table access count`          | メモリテーブルにアクセスした回数です。                         |
| `module`                             | 現在実行中のモジュール名です。                             |
...
| `db_username` | データベースに接続されているユーザー名です。          |
| `elapse_time` | 作業が始まってから経過した時間です。              |
| `waiter type` | 現在ロックを待機しているエンティティのタイプを示します。    |
| `holder type` | 現在ロックを所有しているエンティティのタイプを示します。    |
| `holder desc` | 現在ロックを所有しているエンティティの詳細な説明を提供します。 |
| `waiter desc` | 現在ロックを待機しているエンティティの詳細な説明を提供します。 |
| `query`       | 実行されたSQLクエリです。                  |
| `query_param` | クエリの実行時に使用されたパラメータです。           |
