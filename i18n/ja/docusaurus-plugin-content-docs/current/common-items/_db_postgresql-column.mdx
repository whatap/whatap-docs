...
| 項目                | 説明                                            |
| ----------------- | --------------------------------------------- |
| `holder_relation` | `holder`セッションがロックされているオブジェクト(テーブルまたはインデックス)   |
| `holder_type`     | `holder`セッションの`type` (transaction id、tupleなど) |
| `lock_mode`       | `holder_mode` (exclusive lock、shared lockなど)  |
| `lock_request`    | `waiter_mode` (exclusive lock、shared lockなど)  |
| `state`           | 各セッション(ベックエンドプロセス)の現在の状態                      |
| `waiter_relation` | `waiter`セッションがロックされるのを待っているオブジェクト             |
| `waiter_type`     | `waiter`セッションの`type` (transaction id、tupleなど) |
