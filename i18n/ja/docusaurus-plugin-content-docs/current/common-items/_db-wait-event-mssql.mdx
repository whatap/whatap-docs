...
- `PAGEIOLATCH_EX`：データページを読み取るために排他的ロックを待機します。

- `PAGEIOLATCH_SH`：データページを読み取るために共有ロックを待機します。
...
- `ASYNC_NETWORK_IO`：ネットワークI/O作業中の待機です。

- `SLEEP_*`：`WAITFOR`または`WAITFOR DELAY`文により待機中です。

- `BROKER_*`：Service Brokerメッセージ待機です。

- `LOCK_MANAGER`：ロック管理待機、**SQL Server**のロック管理機に関連する待機です。
