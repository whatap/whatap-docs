...
semanticな単語すべて、正規表現に書き換えできます。 GROKパーサーを使用すると、事前に定義されたnamed regular expressionsを活用できます。 ここで使われた`TIMESTAMP_ISO8601`、`LOGLEVEL`、`DATA`は、GROKが提供するnamed regular expressionsです。 これらの値は、それぞれ次の正規表現に置き換えられてマッチングされます。 
...
- GROKパーサーは`%{SYNTAX:SEMANTIC}`、`%{SYNTAX:SEMANTIC}`二つのパターンをサポートします。
...
2. **_+_** <Cmdname sid="adding" className="uitext" />を選択した後<Cmdname sid="parser" className="uitext" />入力欄で**_JSON_**パーサを選択してください。
   ...
   <ImgLang img='log-1st-json.png' desc='JSONパーサーパターンの登録'/>
...
3. <Cmdname sid="category" className="uitext" />および<Cmdname sid="ITM06439" className="uitext" />、<Cmdname sid="pattern" className="uitext" />を入力してください。 <Cmdname sid="parser" className="uitext" /> <Cmdname sid="add" className="uitext" />ウィンドウのコンポーネントは次の通りです。
   ...
   - <Cmdname sid="pattern" className="uitext" />
     ...
     - JSONパターンの例
       ...
       例のログで**_Prefix_**と**_Postfix_**を`--`に指定し、**_Ignore_**に`url`を指定時`host`、`method`、`status`3つ<Cmdname sid="tag" className="uitext" />のみ生成します。
...
- パーサを登録する前に<Cmdname sid="simulation" className="uitext" />を通じて登録しようとするパターンが正常であるか確認できます。 GROKパーサー登録シミュレーションプロセスと同じです。 [次の文書](#grok-simulation)を参考にしてください。
...
もし、例のようにログの一部のみがJSONフォーマットで構成されている場合は**_Prefix_**と**_Postfix_**を指定してください。 WhaTapログモニタリングは**_Prefix_**と**_Postfix_**間の領域をJSONフォーマットで認識してからパーシングします。
