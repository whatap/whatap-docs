-   **dbcp_pool_enabled** <span class='type'>Boolean</span>
    ...
    JMXを使わずにDBCPのDB Connection情報を追跡するために使用します。

-   **hikari_pool_enabled** <span class='type'>Boolean</span>

    デフォルト値`false`

    JMXを使わずにhikari poolのDB Connection情報を追跡するために使用します。
...
-   **profile_error_jdbc_fetch_max** <span class='type'>Int</span>
    ...
    SQL Fetch Count(`ResultSet.next()`呼び出し件数)が設定した値を超えるとTOOMANY Fetchエラーとして処理します。`0`に設定するとエラー処理しません。
