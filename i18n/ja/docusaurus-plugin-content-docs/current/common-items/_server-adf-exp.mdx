...
**_イベント発生条件_**と**_イベント対象フィルタリング_**で**_選択入力_**モードを選択した状態で、ピリオド(.)が2つ以上のフィールドまたはタグ名を選択した場合は、**_直接入力_**モードを選択し、`${フィールドまたはタグ名}`のような形式でまとめて入力する必要があります。

例えば、`webcheck.ping.status_code`フィールド名と演算子は`!=`、値は`200`を入力する場合、**_直接入力_**モードを選択した後`${webcheck.ping.status_code} != 200`で直接入力してください。
