...
WhaTapの**AWS RDS Log**は**AWS CloudFormation**を通じて設置することができます。 **AWS CloudFormation**についての詳細は[次のリンク](https://docs.aws.amazon.com/ko_kr/AWSCloudFormation/latest/UserGuide/Welcome.html)を参照してください。

### 権限確認

**AWS CloudFormation**を通じてWhaTapの**AWS RDS Log**をインストールするには、まず必須IAM権限を確認してください。
...
1.  WhaTapが提供する**AWSCloudFormation**テンプレートを通じてインストールすることができます。 次のリンクに移った後、**パラメータ**セクションで以下の項目を入力してください。
...
2.  ログモニタリングの詳細設定のために、次のリンクにアクセスしてください。
...
すべての設定を完了した後、**_ログ_**>**_ログ設定_**メニューに入り、ログモニタリングを活性化にしてください。

<ImgLang img='db-log-setting-active.png' desc='ログモニタリングの活性化' />
