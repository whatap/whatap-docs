...
`hooklog_enabled`値を変更した場合は再起動が必要です。`hooklog_enabled` のデフォルト値は`logsink_enabled`です。従って`logsink_enabled=true`として設定してJavaアプリケーションを再起動すると、`hooklog_enabled`は`true`として設定され`hooklog_logback_enabled`も`true`に設定されます。
...
-   **hooklog_logback_enabled** <span class='type'>Boolean</span>
    ...
    logbackフレームワークの収集ログ追跡を有効にするかどうかを設定します。
