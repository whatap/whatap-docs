...
`System.gc()`を呼び出してJVM(Java仮想マシン)のヒープ領域で 動的に割り当てたメモリのうち、不要になったメモリオブジェクト(garbage)を集めて削除することができます。 <Cmdname sid="BTN07740" className="uitext" />ボタンを選択するとGCプロセスが実行され、実行前後のメモリ容量を確認できます。
