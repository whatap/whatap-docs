...
現在のプロジェクトと関連づけられたエージェントを選択すると、フィルタリングすることができます。![アイコン](/img/ic-search-on-press.svg) **_虫眼鏡_**アイコンを選択すると、![number2](/img/number-02.png)領域で**_ヒットマップトランザクション_**チャートが表示します。
...
**_TXトレース_**一覧に詳細情報が含まれています。一覧から目的のトランザクションを選択すると、**_統計_**情報ウィンドウが表示されます。**_統計_**ウィンドウでは、トランザクションのトレースの詳細分析を確認できます。詳細については、[次の文書](trs-profile#details)をご参照ください。
...
**_ヒットマップトランザクション_**チャートを分析する方法についての詳細は、[次の文書](../best-practice-guides/about-apm-hitmap-class)をご参照ください。
