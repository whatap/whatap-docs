...
- どのキーワードでインデックスをかけるべきかわからない場合は、Content検索キーを活用して問題となるキーワードを含むログを識別します。 その後、<Cmdname sid="side_logSetting" className="uitext" />メニューのログパーサ設定を通じて、該当キーワードでパーサを設定してインデックスを生成する方式で、検索速度を向上させることができます。
