...
現在実行中のセッションのSQLクエリ情報を確認するには<Cmdname sid="active_sessions" className="uitext" />セクションの<span class="uitext">query</span>カラム項目を選択してください。 <Cmdname sid="TTL06224" className="uitext" />ウィンドウが表示されます。 SQLクエリ文とPlan情報を確認することができます。
...
-   **_Plan_**: Plan情報を確認するには**DB Name**、**User Name**、**Password**を入力してから、![検索アイコン](/img/ico-btn-search.png)ボタンを選択してください。
    ...
    -   **_JSON_**: Json形式でプラン情報を照会すると、ボトルネックが発生するポイントをすばやく確認できます。

    :::note

    **_JSON_**形式でプラン情報を照会する機能は、DBXエージェント1.6.12バージョン以降でのみ対応されます。
