...
- **whatap.whatap.profile_compile_file_enabled** <span class='type'>Boolean</span>
  ...
  `compile_file`、`compile_string` 関数が追跡されるかどうかを設定します。PHP内部関数である`compile_file`、`compile_string` 関数はスクリプト実行前にコンパイルする関数で、Main、include、requireおよびautoloadスクリプトのコンパイル時点およびスクリプトファイルのパスと実行時間を確認できます。
...
- **whatap.profile_compile_file_basetime** <span class='type'>Millisecond</span>
  ...
  実行時間の基準を設定します。設定した時間を超えて実行される場合のみ、情報を収集します。
...
- **whatap.profile_method_enabled** <span class='type'>Boolean</span>
  ...
  実行時間が`whatap.profile_method_time`の設定値(デフォルト値`1秒`)を超えるユーザ関数の情報をトランザクション詳細情報から確認できます。
...
- **whatap.profile_internal_method_enabled** <span class='type'>Boolean</span>
  ...
  PHP内部関数が追跡されるかどうかを設定します。`whatap.profile_internal_method_time`の設定値(デフォルト値`1秒`)より長くかかる場合のみ追跡します。 
...
- **whatap.profile_internal_method_time** <span class='type'>Milliseconds</span> <!-- 두 개가 어떻게 다른지? 0309 수정완-->
  ...
  PHP内部関数を追跡する際、(`whatap.profile_internal_method_enabled`)関数の実行時間を設定します。 
...
- **whatap.profile_internal_method_param_enabled** <span class='type'>Milliseconds</span>
  ...
  PHP内部関数を追跡する際、(`whatap.profile_internal_method_enabled`)付加的に関数パラメータ情報を収集します。
...
- **whatap.net_udp_flush_start** <span class='type'>Boolean</span>
  ...
  値段が`false`であれば、トランザクションの開始時点を収集しません。
...
- **whatap.net_udp_profile_basetime_enabled** <span class='type'>Boolean</span>
  ...
  値が`true`であれば、設定された応答時間より早いトランザクションのトレース情報を収集しません。
...
- **whatap.net_udp_trace_ignoretime_enabled** <span class='type'>Boolean</span>
  ...
  値が`true`であれば、設定された応答時間がかかるトランザクションおよびトレース情報を収集しません。

- **whatap.net_udp_trace_ignoretime** <span class='type'>Miliseconds</span>

  デフォルト値`30`

  トランザクションおよびトレース情報収集を除くトランザクションの応答時間を設定します。
...
- **whatap.shm_tx_counter_enable** <span class='type'>Boolean</span>
  ...
  収集されていないトランザクションの場合、TPS、平均応答時間、Apdex指標には正常に反映されますが、その他の統計データには反映されません。
...
- **whatap.split_profile_enabled** <span class='type'>Boolean</span>
  ...
  一定量のプロファイルを仮想トランザクションに分離して、より多くのプロファイル情報を収集します。複数のトランザクション一覧に分割表記されます。
...
- **whatap.large_profile_enabled** <span class='type'>Boolean</span>
  ...
  一定量のプロファイルを圧縮および分割転送することで、メモリの増加なしにより多くのプロファイル情報を収集できます。

  :::note

  - 約1万件以下のプロファイル（ステップ）情報を収集できます。より多くの量は、`whatap.split_profile_enabled`オプションの使用をお勧めします。
...
- **whatap.profile_zip_enabled** <span class='type'>Boolean</span>
  ...
  プロファイル情報を圧縮転送します。デフォルト値は、`true`と設定されています。 
