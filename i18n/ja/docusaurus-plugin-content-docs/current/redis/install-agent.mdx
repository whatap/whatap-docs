...
3. 解凍したフォルダに移動し、_whatap.conf_ファイルを確認してください。_whatap.conf_ ファイルにプロジェクトのアクセスキー(`license`)情報、WhaTapサーバー情報(`whatap.server.host`)、DB接続情報(`dbms`、`db_ip`、`db_port`)を入力します。
