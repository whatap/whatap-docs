...
- ***instance***：クラスターに属する各インスタンスの名前または識別子です。

- ***cluster name***：クラスターを識別するためのクラスターの名前です。

- ***flags***：各インスタンスの状態や特性を示すフラグ情報です。

- ***ip***：各インスタンスのIPアドレスです。

- ***link status***：各インスタンスの接続状態です。

- ***slots***：クラスターで使用されるスロット(slot) 情報です。各スロットは、クラスター内のデータの一部を担当します。この情報により、クラスターのデータ分散とロードバランシングの状態を把握できます。

:::note

<<<<<<< HEAD
***Cluster Info***ウィジェットを利用するには、DBXエージェント(*whatap.conf*)の次のオプションを設定する必要があります。詳細については、[次の文書](agent-dbx-settings#cluster-set)を参照してください。
=======
***Cluster Info***ウィジェットを利用するには、DBXエージェント(*whatap.con*)の次のオプションを設定する必要があります。詳細については、[次の文書](agent-dbx-settings#cluster-set)を参照してください。
