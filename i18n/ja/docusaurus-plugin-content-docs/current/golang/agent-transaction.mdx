...
-   **grpc_profile_enabled** <span class='type'>Boolean</span><br/>
    デフォルト値 `true`<br/>gRPC（Google Remote Procedure Call, 以下gRPC）情報の収集有無を設定します。

-   **grpc_profile_stream_client_enabled** <span class='type'>Boolean</span><br/>
    デフォルト値 `true`<br/>client stream method情報の収集有無を設定します。

-   **grpc_profile_stream_server_enabled** <span class='type'>Boolean</span><br/>
    デフォルト値 `true`<br/>server stream method情報の収集有無を設定します。

-   **grpc_profile_ignore_method** <span class='type'>String</span><br/>
    デフォルト値 `agent`<br/>設定した値に対するメソッドを収集しません。複数登録するときは、コンマ（,）を区切り文字として利用してください。
...
-   **grpc_profile_stream_identify** <span class='type'>Boolean</span><br/>
      デフォルト値 `false`<br/>Stream full methodをトランザクション名で収集し、同一のfull methodに対してclientとserverを区切る接頭辞を追加します。
    ```ini title='예시'
...
-   **profile_http_parameter_enabled** <span class='type'>Boolean</span><br/>
    デフォルト値 `false`<br/>gRPC情報の中でhttpパラメータ情報の収集有無を設定します。

-   **profile_http_header_enabled** <span class='type'>Boolean</span><br/>
    デフォルト値 `false`<br/>gRPC情報の中でhttpヘッダー情報の収集有無を設定します。
