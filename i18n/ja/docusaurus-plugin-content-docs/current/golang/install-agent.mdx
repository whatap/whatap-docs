...
WhaTapモニタリングサービスを使用するには、[会員登録](https://www.whatap.io/ja/)後、プロジェクトを作成し、ターゲットサーバーにエージェントをインストールする必要があります。
...
エージェントは、ユーザーアプリケーションのモニタリング情報を内部 IPC（TCP）で受信し、そのデータを WhaTap 収集サーバーへ送信するデータ中継プロセスです。
