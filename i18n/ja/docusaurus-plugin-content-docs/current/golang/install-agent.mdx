...
エージェントは基本**127.0.0.1:6600**TCPサーバーで動作します。Goアプリケーションでデータを受信し、WhaTap収集サーバーにOutbound 6600ポートに転送します。
...
`trace.Init()`, `trace.Shutdown()`関数で初期化および終了を設定してください。`trace.Start()」`,`trace.End()`関数でトランザクションの開始終了を設定してください。
