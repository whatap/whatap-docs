...
-   **grpc_profile_stream_method** <span class='type'>String</span>
    ...
    設定したstream methodを別途のトランザクションで構成します。 複数の項目を登録する場合は、コンマ(,)を区切り文字として使用してください。 長時間続くstream 接続に対して、それぞれのmethod 呼び出しを単独トランザクションとして処理します。 ヒートマップ、トランザクション検索からfull methodで検索できます。
