...
WhaTapエージェントは、エージェントごとに必要な設定を*whatap.con*ファイルに作成します。 エージェントは、環境変数を使用して設定ファイルの位置を把握して読み込みます。

別な設定がない場合、エージェントのパスを*whatap.con*ファイルに実装しました。5秒ごとに、*whatap.con*ファイルをチェックしてリロードします。
