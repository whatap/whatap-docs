...
ホーム画面 > **Featureプロジェク**(<code class='Features'>Features</code>)選択 > ***NGINX*** > ***NGINX ダッシューボード***
...
- Nginx Connections Accepted: NGINXが受信して許可した接続の合計数を示します。 このメトリクスは、サーバーに入ってくるトラフィックをモニタリングし、どれだけのクライアントがNGINXに接続できるかを把握するのに役立ちます。

- Active Connections (accepted): 現在NGINXサーバーが受け入れたアクティブな接続状態をリアルタイムで表示します。 このメトリクスは、NGINXが現在処理中のクライアントとの接続数を示し、サーバの負荷状態を理解するのに役立ちます。

- Handled Request: NGINXサーバーが正常に処理したリクエストの数を追跡します。 このメトリクスは、リクエストがサーバーでどのように処理されたかを反映しており、特定の時間の間に正常に処理されたリクエストの量を確認できます。

- Nginx Requests: NGINXサーバーが受信したリクエストの合計数を表示します。 このメトリクスには、クライアントがサーバーに送信したすべてのリクエストが含まれ、リクエストの量を把握し、サーバーのトラフィックを理解するのに役立ちます。 
