...
4. 次のディレクトリにJDBCドライバーをダウンロードしてください。_/圧縮を解凍したフォルダ/jdbc_
   ...
   データベースサーバーのOSとバージョン用のJDBCドライバーをダウンロードしてください。https://www.cubrid.com/downloads
...
データベース接続用に暗号化されたUIDを生成します。ユーザーファイルは、ユーザーDB(`dba`)とCUBRID Manager(`admin`)の両方に対して実行する必要があります。usernameとpasswordを入力し、シェルスクリプト(またはバッチファイル)を実行します。 

- _uid.sh_シェルスクリプトファイル(または_uid.bat_配置ファイル)は、WhaTapデータベースエージェントをインストールしたパスにあります。
