...
エージェントをインストール案内に従ってインストールすると、_whatap.agent.java.tar.gz_ファイルをダウンロードできます。 圧縮を解凍すると_whatap.agent.proxy-`X.Y.Z`.jar_ファイルをWhaTap収集サーバーと通信可能なサーバーの場所に保存してください。 インストール済みのエージェントがある場合は、その場所のファイルを利用できます。
...
_whatap.agent.proxy-`X.Y.Z`.jar_ファイルを適切なパスにコピーしたら、WhaTap収集サーバーIPとポート番号を設定するために_proxy.conf_ファイルを作成してください。 _whatap.agent.proxy-`X.Y.Z`.jar_ファイルがあるパスは`$PROXY_HOME`となります。
...
ソフトウェアプロキシは、`java`コマンドを使用して実行したり_proxy.sh_(_proxy.bat_)ファイルを作成して実行してください。
...
_proxy.sh_(_proxy.bat_)ファイルを作成して実行する場合、`$ chmod +x proxy.sh`コマンドを使用して実行権限を付与してください。
...
次を参照して_haproxy.cfg_ファイルを設定してください。
...
次を参照して_nginx.conf_ファイルを設定してください。
