...
MySQLの`performance_schema`で提供するSQL統計データ(遂行回数、実行時間、Rowの数など)で`performance_schema.events_statements_summary_by_digest`ビューを通じて収集します。 
...
2.  モニタリングアカウントからそのデータを読み取れるように照会権限を設定してください。

    ```bash
    grant select on performance_schema.* to whatap;
    ```

3.  DBXエージェントにオプションを設定してください。
...
1時間おきに`events_statements_summary_by_digest`からデルタ値を計算して収集します。 したがって、設定してから1時間後から収集を開始します。
...
| `created_tmp_disk_tables` | コマンドの実行中にサーバーが作成したディスクの臨時テーブル数の合計、回数が高い場合は、sortメモリサイズの増設を考慮してください。     |
| `created_tmp_tables`      | コマンドの実行中にサーバーが作成した内部臨時テーブル数の合計                                         |
| `select_full_join`        | インデックスを使用していないtablescanの数、0でなければインデックス確認が必要です。                         |
