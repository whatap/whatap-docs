...
```bash
create user DB_User identified by 'DB_PASSWORD'; # パスワードを入力してください。
...
| ^           | `grant select on performance_schema.* to whatap;`                                                | アクティブセッションの収集 / <Cmdname sid="side_dbLockTree" className="uitext" /> / <Cmdname sid="TTL06865" className="uitext" />                     | `performance_schema.data_locks`<br/>`performance_schema.threads`<br/>`performance_schema.events_statements_summary_by_digest`など |
