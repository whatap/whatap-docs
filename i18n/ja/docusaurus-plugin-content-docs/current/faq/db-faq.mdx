...
MySQL 8.0のデフォルト認証プラグインが、`caching_sha2_password`であるため発生する可能性があります。`caching_sha2_password`を使用するには、SSLで保護されたセキュリティ接続を使用するか、RSAセキュリティを使用した非暗号化接続を使用する必要があります。この問題を最も簡単に解決する方法は、パスワード生成時に、以前の`mysql_native_password`方式を使用することです。
