...
-   **CloudFormation Stack Name**
    ...
    WhaTap Forwarderのインストール及び除去に活用されるCloudFormation Stack名前を指定します。
...
-   **Timeout** <span class='type'>Second</span>
    ...
    WhaTap Forwarderの休止期間(秒)を指定します。ログの転送リクエストがないときにLambda Functionが削除されるまでの時間を指定します。 

-   **UseReservedConcurrency** <span class='type'>Boolean</span>

    デフォルト値`false`

    ログの安定的な転送のため、WhaTap Forwarderに割り当てる最大Functionの個数の指定可否を設定します。

-   **ReservedConcurrency** <span class='type'>Second</span>

    デフォルト値`10`

    `UseReservedConcurrency`の値が`true`である場合、WhaTap Forwarderに割り当てるFunctionの個数を指定します。AWSアカウントごとにデフォルトで割り当てられるFunctionの数は1,000個です。ユーザーが使用できるFunctionの総個数はアカウントあたりの総割り当てFunction個数(1,000)で`ReservedConcurrency`の設定値を引いたものに制限されます。
...
3.  詳細画面右側の**Description**領域で![number 3](/img/number-03.png)**Function ARN**(**_WhaTap Forwarder ARN_**)を確認することができます。

4.  次のステップのためにコピーしてください。
...
2.  コンソール探索ウィンドウで**_Roles_**を選択してから、[number 1](/img/number-01.png)**Create role**ボタンを選択してください。
...
WhaTap**_エージェント設定_**>**_インストール案内_**セクションの**_AWS Resource Log照会およびサブスクリプション_**タブで上段の**_AWS CloudWatch Log Group_**を選択して進行してください。 

<ImgLang img='aws-log-cloudwatch-subs.png' desc='AWS CloudWatch Logの照会およびサブスクリプション'/>

1.  AWS Logがインストールされた![number 1](/img/number-01.png) **_AWS Region_**を選択してください。 

2.  [既存のステップ](#whatapforwarder-arn)で獲得した**WhaTap Forwarder ARN**をコピー後![number 2](/img/number-02.png) **_AWS IAM Role ARN_**に貼り付けてください。
...
WhaTap**_エージェント設定_**>**_インストール案内_**セクションの**_AWS Resource Log照会およびサブスクリプション_**タブで上段の**_AWS S3 Bucket_**を選択して進行してください。 

<ImgLang img='aws-log-s3-subs.png' desc='AWS S3 Bucketの照会およびサブスクリプション'/>
...
モニタリングが活性化されたら**_ダッシュボード_**>**_ライブテール_**メニューで流入されるログの出力を確認することができます。

:::note

プロジェクトに対する**修正権限**がある場合に限ってログモニタリングをアクティブにすることができます。権限の詳細については、[次の文書](../project/project-structure)を参考にしてください。
