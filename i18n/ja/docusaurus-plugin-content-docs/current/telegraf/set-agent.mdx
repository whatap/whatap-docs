...
WhaTapは、モニタリング情報の収集対象のインフラサーバーの識別のために、基本的にサーバーから収集した情報を使用します。 
...
-   **object_name** <span class='type'>String</span>
    ...
    | 名称         | 説明                                                        |
    | ---------- | --------------------------------------------------------- |
    | {type}     | app_nameに設定された値を使用します。 デフォルト値は、Telegrafです。                |
...
-   **telegraf_delta_fields** <span class='type'>String</span>
    ...
    e.g. nginx.accepts,nginx.requests nginxは、accepts, requests値の増加値をacceptsDelta, requestsDeltaフィールド名に追加します。
