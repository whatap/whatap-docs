...
3.  圧縮して解凍したフォルダに入り、_whatap.conf_ファイルを確認してください。 _whatap.conf_ プロジェクトのアクセスキー情報、WhaTapサーバー情報、db接続情報を入力してください。
    ...
    **MySQLサーバとエージェントがlocalhostにインストールされる場合(default 3306 port)**
...
-   JDBCドライバー8.xバージョン以降を利用する場合は、接続URLに`serverTimezone`設定が必要な場合は、次のようにオプションを設定してください。
...
-   SSLを利用する場合は、次のようにオプションを設定してください。
...
create user whatap identified by 'DB_PASSWORD';#希望するpasswordを入力してください。
