...
- **_Read Clients_** (`readers$activeClients$globalLock`)
  ...
  MongoDBのグローバルロック(global lock)で現在読み取り作業を行っているクライアントの数を測定したウィジェットです。データベースサーバーが読み取り操作のためにどれだけのクライアントを処理しているかを把握できます。データベースの性能をモニタリングし、レスポンスタイムを追跡するのに役立つ指標です。過度な読み取り操作が発生すると、性能低下の原因になる可能性があります。

- **_Write Clients_** (`writers$activeClients$globalLock`)

  現在MongoDBのグローバルロック(global lock)で現在書き込み作業を行っているクライアントの数を測定したウィジェットです。データベースサーバーが書き込み操作のためにどれだけのクライアントを処理しているかを把握できます。データベースの性能をモニタリングし、レスポンスタイムを追跡するのに役立つ指標です。過度な書き込み操作が発生すると、性能低下の原因になる可能性があります。

- **_Queued Read Requests_** (`readers$currentQueue$globalLock`)

  現在システムで待機中の読み込み作業を処理するために、グローバルロック(global lock)の待機中のクライアントの数を測定したウィジェットです。データベースの性能とスケーラビリティを評価できる情報を提供します。待機中の読み込みリクエスト作業が多いほど、クライアントのリクエストに対するレスポンスタイムが長くなる可能性があり、処理量が限界に達する可能性があります。
...
- **_Queued Write Requests_** (`writers$currentQueue$globalLock`)
  ...
  現在システムで待機中の書き込み作業を処理するために、グローバルロック(global lock)の待機中のクライアントの数を測定したウィジェットです。データベースの性能を評価し最適化するための必須指標です。待機中の書き込み作業が増えると、システムの性能が低下するか、レスポンスタイムが増加する可能性があります。
...
- **_Network Out_** (`bytesOut$network`)
  ...
  クライアントまたはその他のインスタンスの接続を介してサーバーが送信した総バイト数です。データベースサーバーが外部からのリクエストを受けてレスポンスを返すときに送信したデータの総量を意味します。 データベースサーバーから発生するネットワークトラフィックを特定することができ、これにより、ネットワーク帯域幅やサーバーリソースの使用に最適化を進めることができます。さらに、高いネットワーク トラフィックは、レスポンスタイムの増加などの性能問題に関連しています。
