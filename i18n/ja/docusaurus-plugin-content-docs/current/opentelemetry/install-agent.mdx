...
エージェント設定(*whatap.con*)ファイルへのアクセスキーと収集サーバーのIPアドレスを設定します。
...
環境変数と*whatap.con*ファイルをすべて設定した場合、*whatap.con*ファイルの設定値が優先されます。
...
Kong Gatewayの組み込みOpenTelemetryプラグインを設定すると、OpenTelemetry Collectorを通じてトランザクション情報を確認できます。

OpenTelemetryプラグインでOpenTelemetry Collectorのエンドポイントを指定します。CollectorにWhatap-otelエージェントが接続されると、トレース情報が収集されます。
...
1. OpenTelemetryプラグイン設定 

    OpenTelemetry CollectorのエンドポイントURLを登録します。

    CollectorのOTLPレシーバーのデフォルト設定は以下の通りです。
