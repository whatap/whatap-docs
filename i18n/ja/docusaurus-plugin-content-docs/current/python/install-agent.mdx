...
<FiletreeItem name="net / pack" type="folder">UDP通信に関連するファイルで構成されたディレクトリです。収集したデータをサーバーに送信する役割を果たします。</FiletreeItem>
...
<FiletreeItem name="paramkey.txt" type="file">
  ...
  追跡したトランザクションのトレース情報として収集したHTTPとSQLデータのパラメータ情報を確認するために使用します。セキュリティキーをファイルに保存し、実際に収集されたデータをブラウザで確認したい時、ファイルに保存しておいたセキュリティキーを入力することで照会できます。ファイルの内容を直接変更して、セキュリティ キーを修正できます。関連オプションの詳細については、次の文書を参照してください。
...
アクセスキー及び収集サーバーのIP設定のために`whatap-setting-config`コマンドを実行してください。
...
アプリケーションサーバーの環境によって、適用する方法が異なる場合があります。ユーザーの環境に合わせた適用方法を進めてください。
...
**Supervisor**環境では、適用するために_supervisor.conf_ファイルに次の設定を追加してください。
