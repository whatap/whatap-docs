...
- **profile_sql_param_enabled** <span class='type'>Boolean</span><ScrollToLink anchor='profile-sql-param' />
  ...
  SQLパラメータの収集をアクティブにします。 パラメータは、別途のセキュリティキーを入力しないと照会できません。 セキュリティキーは、エージェントインストールパスの*paramkey.txt*ファイルに6桁で指定します。 *paramkey.txt*が存在しない場合は自動生成され、randomキーが設定されます。
