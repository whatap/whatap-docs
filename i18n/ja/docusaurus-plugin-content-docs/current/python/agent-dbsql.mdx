...
-   **profile_sql_param_enabled** <span class='type'>Boolean</span>
    ...
    SQLパラメータの収集をアクティブにします。 パラメータは、別途のセキュリティキーを入力しないと照会できません。 セキュリティキーは、エージェントインストールパスの_paramkey.txt_ファイルに6桁で指定します。 _paramkey.txt_が存在しない場合は自動生成され、randomキーが設定されます。
