...
WhaTap iOS モバイルモニタリングサービスを利用するには、[会員登録](https://www.whatap.io/ko/signup/)後にプロジェクトを作成し、iOS アプリケーションに WhaTap iOS エージェントを適用する必要があります。
...
```swift
let agent = WhatapAgentBuilder()
  .setProjectKey("プロジェクトキー")     // 必須
  .setCode(3380)                    // 必須
  .setServerUrl("15.165.146.117")   // 必須
...
```objectivec
WhatapAgentBuilder *builder = [[WhatapAgentBuilder alloc] init];
[builder setProjectKey:@"プロジェクトキー"];    // 必須
[builder setCode:3380];                     // 必須
[builder setServerUrl:@"15.165.146.117"];   // 必須
...
* **モニタリングを開始する**
  ...
  [WhaTap モニタリングサービス](https://service.whatap.io) ページに移動し、ブラウザーモニタリングを開始してください。  
  作成したプロジェクトを選択し、<Cmdname sid="side_dashboard" className="uitext" /> &gt; <Cmdname sid="side_dashboard" className="uitext" /> メニューに移動します。 