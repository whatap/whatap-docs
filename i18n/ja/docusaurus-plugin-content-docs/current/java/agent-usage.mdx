...
- **sigar_enabled** <span class='type'>Boolean</span>
  ...
  - Java 8 未満またはJava 1.6 以下のバージョン環境では、`sigar_enabled=true`オプションを追加してsigarライブラリを有効にし、パフォーマンス指標を収集できます。

  - アプリケーション環境によってCPU指標が**-1**と表示される現象がある可能性があります。WhaTapが提供する勧告事項を確認し、現状を解決できます。[次の文書](agent-troubleshooting#cpumetrics)を参照してください。
