...
- `BP_WHATAP_ENABLED`
  ...
  WhaTapエージェントの有効の有無を設定します。 値が`false`の場合、インストールは続行されますが、モニタリングが停止した状態で開始されます。 後で、[WhaTapモニタリングサービス](https://service.whatap.io)の***管理*** > ***エージェント設定***メニューから`true`に変更できます。
