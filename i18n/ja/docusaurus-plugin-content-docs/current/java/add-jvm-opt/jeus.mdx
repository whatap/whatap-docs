...
[WhaTapモニタリングサービス](https://service.whatap.io/)にログインし、プロジェクトを選択してください。**_アプリケーション_** > **_インスタンス性能分析_**メニューを選択し、**_環境変数の実行_**タブを選択してください。**_whatap.name_**と**_whatap.type_**項目を確認してください。**_whatap.name_**項目の最後の要素は、コンテナ名である必要があります。**_whatap.type_**項目には、アプリケーションサーバーの種類を指定する必要があります。
