...
[WhaTapモニタリングサービス](https://service.whatap.io/)にログインし、プロジェクトを選択します。<Cmdname sid="TTL00901" className="uitext" /> > <Cmdname sid="instance_performance_analysis" className="uitext" />メニューを選択し、<Cmdname sid="runtime_boot_env" className="uitext" />タブを選択します。**_whatap.name_**と**_whatap.type_**項目を確認してください。**_whatap.name_**項目の最後の要素は、コンテナ名にしてください。**_whatap.type_**項目には、アプリケーションサーバーの種類を指定してください。
