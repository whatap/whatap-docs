...
id: ibm-bluemix
title: IBM BlueMix
description: IBM BlueMix環境でWhaTapブエージェントの適用を完了してください。
tags:
  - Java
  - IBM BlueMix
---

**IBM BlueMix**を利用して配布する場合、WhaTapエージェントを適用する方法です。 

[IBM BlueMix](https://console.bluemix.net/docs/apps/index.html%E2%80%8B)は、コンテナで**WebSphere Liberty**環境を提供します。**Liberty**は、**WebSphere Application Server**とは異なる軽量化環境で**Spring Boot**が動作する方式と似ています。

{@include: _pass-note.mdx}

以下は設定環境の例です。 [ガイド](https://cloud.ibm.com/docs)による環境を構成する場合、ローカル開発環境で生成するファイルです。
...
1.  `${APP_HOME}`で_src/main/resources/whatap-agent/_ディレクトリを作成し、jarファイル、confファイルをコピーしてください。
...
2.  _${APP_HOME}/manifest.yml_ファイルにオプションを追加してください。ymlファイルですので、スペース、インデント基準に沿って作成してください。
...
-   _whatap.conf_設定は、PaaSではなく環境と同様に適用します。適用後、エージェント名の識別が困難な場合がありますので、状況に応じたエージェント名を適用してください。
-   エージェントネーミングの詳細については、[次の文書](../agent-name)を参考にしてください。
:::
