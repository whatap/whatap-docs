...
Java1.5バージョンは、`-javaagent`オプションを提供します。 ただ、外部スレッドの情報を照会するのに制約があり、一部の機能に制約があります。
...
Java1.4バージョンは、`-javaagent`オプションがありません。別の方法でWhaTapエージェントをインストールする必要があります。_whatap.agent.jar_ファイルもJava1.4バージョンを使用してください。

> whatap.java14.tracer-`X.Y.Z`.jar

### boot.jar作成

_setup.sh_ファイルを利用して_boot.jar_ファイルを作成してください。 もし[JAVA_HOME]を入力しない場合、現在のパスはJDKの位置を自動的に認識します。
