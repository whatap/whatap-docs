...
WhaTapのJavaモニタリングでは、JVMメモリのヒープ占有オブジェクト現況(ヒープメモリ上のオブジェクト別サイズ)を照会できる機能を標準として提供します。 <Cmdname sid="app_upper" className="uitext" /> &gt; <Cmdname sid="instance_performance_analysis" className="uitext" />メニューから<Cmdname sid="heap_histogram" className="uitext" />タブを選択します。
