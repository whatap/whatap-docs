...
エージェント設定は、*whatap.conf*ファイルを編集するか<Cmdname sid="management" className="uitext" /> &gt; <Cmdname sid="agent_configuration" className="uitext" />メニューで確認できます。詳しい内容は、[次の文書](set-agent)を参考にしてください。
