...
他の設定がない場合、エージェントは(*whatap.agent-X.Y.Z.jar*)にあるディレクトリから*whatap.con* ファイルを実装します。5秒ごとに*whatap.con*ファイルの確認してリロードします。
...
<<<<<<< HEAD
- `whatap.home`：既定値は_whatap.agent.jar_ファイルへのパスを入力します。
=======
...
WhaTapエージェントを初めて実行すると、次のようにどのjarファイルがエージェントとして使用され、どのパスで*whatap.con*ファイルをロードされたのかを確認できます。

![Java start screen](/img/set-java-agent-start.png)

## サーバー接続およびデータ送信

WhaTapエージェントは、*whatap.con*ファイルに設定したサーバーアドレスに接続します。
...
=======
1つのサーバーで複数のアプリケーションを実行する場合、各アプリケーション別の設定ファイルを個別に管理する場合は、*whatap.con*ファイル名を変更し、起動スクリプトにJVMオプションを追加します。
>>>>>>> cf64a884022eddf39df63a56ed06aed971286cd0
...
*whatap.con*設定ファイルは、`${WHATAP_HOME}`パスに配置する必要があります。
