...
-   _paramkey.txt_ `Javaエージェント2.2.2バージョン以前`
    ...
    Javaエージェント2.2.2バージョン以前から2.2バージョンにアップデートする場合、既存の_paramkey.txt_ファイルのキー値が_seuciry.conf_ファイルのキー値として自動的に適用されます。例えば、_paramkey.txt_に`FEDCBA`を使用している場合、2.2.2バージョンにアップデートすると、_security.conf_ファイルには次のように適用されます。
    ...
    _paramkey.txt_ファイルが存在しない場合、_security.conf_ファイルのキー値は、**WHATAP**というキー値で自動生成されます。
