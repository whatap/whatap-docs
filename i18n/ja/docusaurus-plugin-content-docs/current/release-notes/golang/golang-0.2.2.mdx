...
- <Status>Change</Status>エージェント設定方法の変更
  ...
  - プロセス単位で*whatap.con*ファイルを設定するように変更されました。

  - プロセス実行前に*whatap.con*ファイルを事前に作成する必要があります。
...
- <Status>Change</Status>エージェントのIPC通信をUDPからTCPに変更
  ...
  1. ユーザーアプリケーションの*whatap.con*ファイル変更
  ...
  1. whatap-agentサービスの*whatap.con*ファイル変更
...
- <Status>Change</Status>エージェントの命名ルールの変更
  ...
  - 同じプロセスと引数を使用する場合、エージェント名が重複する可能性があります。 *whatap.con*ファイルに`app_name`オプションをユーザーが設定できます。 詳細は、[次の文書](../../golang/install-agent#duplicate_appname)を参照してください。 設定を完了した後は、アプリケーションを再起動する必要があります。
