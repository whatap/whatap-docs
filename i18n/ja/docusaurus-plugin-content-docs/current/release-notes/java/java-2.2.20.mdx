...
-   <Status>Feature</Status>`biz_exceptions_status`オプション追加
    ...
    このオプションを適用すると、ヒートマップinfo(青)で表示し、エラー統計を追加します。
...
-   <Status>Deprecated</Status>Jedis, Tomcat, JMX metric指標(<code>java_jedis</code>, <code>perfx_jmx_interval</code>, <code>perfx_tomcat_enabled</code>)収集の中断
