...
`exception_alert_enabled`オプションを`true`に設定することで機能を有効にし、`exception_alert_stack_size`オプションを指定した値に合わせてエラースタックを通知メッセージを受け取ることができます。

通知が発生すると`exception_alert_silent_time`オプションで指定された期間、エラー通知を無効にすることができます。
