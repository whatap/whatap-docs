...
-   トレースURL名の修正機能の追加
    ...
    # class, method, string, returnのうち1つを選択
    hook_tx_name_mode=method
    ```

    例）特定URLの呼び出し + 特定methodを呼び出す方法

    -   `io.home.test.TestController.test1`に登録したURLの呼び出し時、`test100000()`メソッドを呼び出す場合：`/api/100000+test1+test100000`
...
-   収集するSQLデータの無視機能の追加
    ...
    ```ini title='whatap.conf'
    # 登録したSQL hash値と一致するSQL文は無視
    ignore_sql_hash_set=
    ```

-   特定サービスで特定のexceptionの無視機能を追加
...
-   Javaエージェント組み込みオープンソースプラグインの**webflux-5.x**、**webflux-6.x**アップデート
    ...
    **spring-r2dbc**と連携追跡するため**spring-webflux**プラグインを修正
...
-   HttpURLConnection、WebLogic URLConnectionを最初の呼び出し時にも追跡できるように機能を追加
    ```ini title='whatap.conf'
    # デフォルト値
...
-   **Webflux**使用時に**RemoteIP**ベースのユーザーカウントバグを修正
