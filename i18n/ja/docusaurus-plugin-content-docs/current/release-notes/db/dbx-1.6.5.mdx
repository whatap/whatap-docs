...
- _whatap.conf_ファイルの`slow_query_log`オプションの大文字と小文字の区別しないように修正
...
- `pageSize`データベースパラメータの内容を追加、CUBRID `tps`、`qps`、`psize`、`executions`メトリクス追加、CUBRID `broker`がbroker1またはｍquery_editorでない場合、_whatap.conf_ファイルの`broker=`に記入
