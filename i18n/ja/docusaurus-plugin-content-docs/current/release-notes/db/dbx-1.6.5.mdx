...
- *whatap.conf*ファイルの`slow_query_log`オプションの大文字と小文字の区別しないように修正
...
- **Aurora PostgreSQL**：`instance_name`メトリクスを取得するために追加していた*whatap.conf*ファイルの`rds_instance={instance_name}`オプションを`cloud_watch_instance`に変更
...
**_テーブルスペースサイズ_**メニューにtempファイルサイズの検索機能を追加
...
- テーブルサイズの増分を追加(xcub エージェントのバージョン 0.9.6 以降が必要)
...
- `pageSize`データベースパラメータの内容を追加、CUBRID `tps`、`qps`、`psize`、`executions`メトリクス追加、CUBRID `broker`がbroker1またはｍquery_editorでない場合、*whatap.conf*ファイルの`broker=`に記入
...
- `tps`、`qps`メトリクスが逆転になる内容を修正

- `broker_count`メトリクスを追加

## Redis

`active session`収集のon/off機能を追加(*whatap.conf*ファイルに`active_session=true/false`を追加)
