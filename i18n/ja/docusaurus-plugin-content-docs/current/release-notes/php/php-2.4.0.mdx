...
マルチトランザクション（`whatap.mtrace_enabled=true`）設定が活性化されると、`curl_exec()`関数の前にWhaTap Header情報を設定して、以前ユーザーのHeader情報が初期化されるエラーがあります。 

`curl_exec()`関数呼び出し前にWhaTap Headerを設定する機能を削除し、`curl_init()`関数呼び出し後に設定するよう修正しました。 

`curl_exec()`関数呼び出し前にユーザーHeaderを設定する場合、先に設定されていたWhaTap Headerは削除されます。 

マルチトランザクションを行うためには、以下のガイドに従ってPHPソースコードで直接WhaTap Headerを追加する必要があります。 
...
-   `curl_init()`関数呼び出し後、内部では`curl_setopt()`を呼び出してwhatap headerを設定します。
-   `false`：WhaTap Headerの自動設定機能を停止します。 
