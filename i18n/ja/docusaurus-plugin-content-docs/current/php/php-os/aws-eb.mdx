...
_Elastic Beanstalk Configuration files_(_.ebextensions_)を使用してインストールします。 

_.ebextensions_ディレクトリの配下の_config_ファイルを追加し、新しいバージョンをデプロイします。次に、`commands`コマンドを実行します。
...
1.  Beanstalkに新しいバージョンをデプロイします。PHPモニタリングエージェントがインストールされます。
