...
Custom Dockerで実行するには、`flex`環境と`custom`ランタイムで設定された*app.yaml*ファイルと同じ場所に_Dockerfile_を作成する必要があります。GoogleAppエンジンの基本php Docker画像を参照してください。
...
Google Cloud SDKを使用して設定された*app.yaml*ファイルと_Dockerfile_をデプロイしてください。
