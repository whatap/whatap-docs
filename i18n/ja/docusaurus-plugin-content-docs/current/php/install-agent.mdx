...
<FiletreeItem name="whatap-php(whatap_php)" type="file">エージェントは、トレーサーからUDPに送信された情報を収集サーバーに送信するプログラムです。</FiletreeItem>
...
<FiletreeItem name="whatap-install-yyyymmdd.log" type="file">インストールプロセスのログファイルです。基本の保存パスは*/usr/whata/php/logs*です。日付順にファイルが作成されます。基本的に7日間のログが保存されます。</FiletreeItem>

<FiletreeItem name="whatap-boot-yyyymmdd.log" type="file">エージェントのログファイルです。基本の保存パスは、*/usr/whata/php/logs*です。日付順にファイルが作成されます。基本的に7 日間のログが保存されます。</FiletreeItem>

<FiletreeItem name="install.sh" type="file">インストールスクリプトです。PHP拡張モジュール(*whatap.so *)をApacheまたはPHP-FPMにインストールし、whatap-phpサービスのインストールを進めます。詳細は[次の文書](agent-troubleshooting#extension-modul-manual-setting)を参照してください。</FiletreeItem>
