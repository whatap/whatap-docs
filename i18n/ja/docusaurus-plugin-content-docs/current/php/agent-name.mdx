...
-   **whatap.app_name** <span class='type'>String</span>
    ...
    アプリケーションを識別するためのエージェント名（ONAME）の構成要素として`object_name`の`{type}`に該当する値です。  

{@include: ../common-items/_php-restart-whatap-service.mdx}

-   **whatap.app_process_name** <span class='type'>String</span>

    アプリケーションを識別するためのエージェント名 (ONAME) の構成要素です。アプリケーションプロセス名でアプリケーションサーバーのCPU、Heap Memoryなどを収集する対象プロセスを設定します。`object_name`の`{process}`に該当する値です。  
