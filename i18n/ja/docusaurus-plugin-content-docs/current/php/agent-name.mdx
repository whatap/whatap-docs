...
-   **whatap.app_name** <span class='type'>String</span>
    ...
    アプリケーションを識別するエージェント名(ONAME) 体系に使用されるアプリケーション名で、`object_name`の`{type}`に対応する値です。  

{@include: ../common-items/_php-restart-whatap-service.mdx}

-   **whatap.app_process_name** <span class='type'>String</span>

    アプリケーションを識別するエージェント名(ONAME) システムに使用されるアプリケーションプロセス名です。これを通じて、アプリケーションサーバのCPU、HeapMemoryなどを収集する対象プロセスを設定することができます。`object_name`の`{process}`に該当する値です。  
