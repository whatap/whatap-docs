...
参照時間('stime'、'etime')は1時間以内に制限され、ms(millisecond) 単位です。
...
WhaTapは、直前5分以内にトランザクションを行ったユーザをカウントし、**同時ユーザ**を測定します。
...
- **filterval**: 検索条件 (`filterkey`が`caller_url` または`url`の場合、base64としてエンコード)
