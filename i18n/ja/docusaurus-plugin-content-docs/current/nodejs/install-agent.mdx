...
<FiletreeItem name="whatap.conf" type="file">エージェント設定ファイルのサンプルです。そのファイルをコピーして、アクセスキーを入力してください。</FiletreeItem>

<FiletreeItem name="README.md" type="file">エージェントのインストールに関する案内文書です。</FiletreeItem>

<FiletreeItem name="package.json" type="file">npmモジュール環境構成ファイルです。</FiletreeItem>

<FiletreeItem name="index.js" type="file">main export宣言ファイルです。</FiletreeItem>

<FiletreeItem name="paramkey.txt" type="file">SQL変数、HTTPクエリの照会、Thread中止機能を利用する時に入力するセキュリティキーを保存します。</FiletreeItem>

<FiletreeItem name="lib" type="folder">エージェント、トレーサープログラムです。</FiletreeItem>
