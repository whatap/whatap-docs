...
function updateOrCreateMDXDocument(newFeatures) {
  ...
  // MDX 파일이 존재하는지 확인하고, 없으면 새로 생성
  if (!fs.existsSync(mdxFilePath)) {
      fs.writeFileSync(mdxFilePath, ''); // 빈 파일 생성
  } else {
      ...
      content.trim().split('\n').forEach(line => {
          ...
          if (currentFeature && currentDate && currentDetail) {
              ...
              existingFeatures[key].details.push(currentDetail); // 기존 기능의 세부 정보 추가
          }
      });
  }
  ...
  Object.keys(newFeatures).forEach(key => {
      const newFeature = newFeatures[key];
      const { feature, date, versions } = newFeature;
      const featureKey = `${feature}_${date}`;

      if (!existingFeatures[featureKey]) {
      ...
      // 새로운 기능의 세부 정보를 기존 기능에 추가
      versions.forEach(version => {
          version.details.forEach(detail => {
              if (!existingFeatures[featureKey].details.includes(detail)) {
                  existingFeatures[featureKey].details.push(detail);
              }
          });
      });
  });

  createOrUpdateMDXDocument(mdxFilePath, existingFeatures);
