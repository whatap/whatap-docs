        ...
        try {
            ...
            $('section.remark-sectionize-h2').each((index, element) => {
                ...
                $(element).find('h3').each((index, h3Element) => {
                    ...
                    const fileName = `./crw-data/crwd-service/_import-${lastUrl2}-${productName}.mdx`.trim();
                    ...
                    updateMDXContent(fileName, mdxContent, version, date, featuresByFeatureName, productName, lastUrl2);
            ...
            const newFilesMDX = `./crw-data/crwd-service/_import-new-files-${lastUrl2}.mdx`; // newFilesMDX 정의
...
function updateMDXContent(fileName, existingContent, version, date, newContent, productName, lastUrl2) {
    let updatedContent = existingContent;

    const contentSet = new Set();

    const contentTest = `### <code class='changelog-service'>${version}</code> <code class='changelog-date'>${date}</code>`;
    ...
    const backupFileName = `./crw-data/backup2/_import-backup-${lastUrl2}-${productName}.mdx`;
