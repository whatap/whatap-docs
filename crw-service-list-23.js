...
const urls = [
    'https://docs.whatap.io/release-notes/service/service-1_92_0',
    'https://docs.whatap.io/release-notes/service/service-1_94_0',
    'https://docs.whatap.io/release-notes/service/service-1_96_0',
];
        ...
        try {
            ...
            $('section.remark-sectionize-h2').each((index, element) => {
                ...
                $(element).find('h3').each((index, h3Element) => {
                    ...
                    const fileName = `./crw-data/crwld-service-23q1/_import-23q1-${lastUrl2}-${productName}.mdx`.trim();
            ...
            Object.keys(productFiles).forEach(productName => {
                const filesList = productFiles[productName].map(file => {
                    ...
                    return `import ${productName}_23q1 from ".${cleanFileName}";\n\n<${productName}_23q1 />\n`;
                }).join('\n');
                const cleanProductName = productName.replace(/\u200B/g, '');
                productFilesContent += `## \`${cleanProductName}\`\n\n${filesList}\n`;
            });

            const newFilesMDX = `./crw-data/crwld-service-23q1/_import-new-files-${lastUrl2}.mdx`; // newFilesMDX 정의
...
function updateMDXContent(fileName, existingContent, version, newContent, productName, lastUrl2) {
    ...
    const backupFileName = `./crw-data/backup/_import-backup-23q1-${lastUrl2}-${productName}.mdx`;
