...
// Axios를 사용하여 웹 페이지 HTML 가져오기 11-4
axios.get(url)
    .then(response => {
        // Cheerio를 사용하여 HTML 파싱
        const $ = cheerio.load(response.data);
        ...
        $('section.remark-sectionize-h2').each((index, element) => {
            ...
            h3Elements.each((index, h3Element) => {
                ...
                // h3 다음 h4
                const h3nextH4 = $(h3Element).next('h4').first().text().trim();;
                console.log(h3nextH4);
...
// Axios를 사용하여 웹 페이지 HTML 가져오기 14
axios.get(url)
    .then(response => {
        // Cheerio를 사용하여 HTML 파싱
        const $ = cheerio.load(response.data);
        ...
        $('section.remark-sectionize-h2').each((index, element) => {
            ...
            h3Elements.each((index, h3Element) => {
                ...
                // <h3>가 존재할 때
                // <code class="Feature">가 발견되었으면 처리
                if ((h3nextH4.length === 0) && (featureInP.length > 0 || featureInUl.length > 0)) {
