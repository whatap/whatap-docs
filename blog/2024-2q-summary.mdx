...
와탭 2024년 2분기 `공통` 부분의 주목할 만한 신규 기능으로 ~을 소개합니다.
...
와탭 2024년 2분기 `Application` 부분의 주목할 만한 신규 기능으로 ~을 소개합니다.
...
와탭 2024년 2분기 `Server` 부분의 주목할 만한 신규 기능으로 ~을 소개합니다.
...
컨테이너 맵 상세 내 리소스 관계를 시각적으로 확인할 수 있
...
**Oracle Pro**가 3분기 정식 출시를 앞두고 베타 서비스가 시작되었습니다. **Oracle Pro**는 데이터베이스 서버의 메모리에 직접 접근해 성능 지표를 모니터링하는 기술을 적용하였습니다. 최신 데이터를 반영하여 정확한 성능 정보를 1초에 최대 20번까지 모니터링할 수 있습니다. 디스크 I/O를 거치지 않고 메모리에 직접 접근하여 시스템에 부하가 없습니다. 특정 쿼리의 실행 시간, 버커 캐시 상태 등 정밀하게 수집한 성능 정보를 바탕으로 문제를 빠르고 정확하게 분석할 수 있습니다.
